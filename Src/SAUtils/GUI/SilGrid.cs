...
public class SilGrid : DataGridView {
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        for (int size = 256; size >= 0; size -= 2) {
            using(Font fnt = FontHelper.MakeFont(family.Name, size, FontStyle.Bold)) {
                ...
                if (height < rc.Height) {
                    using(StringFormat sf = GUI.Utils.GetStringFormat(true))
    ...
    /// ------------------------------------------------------------------------------------
    public void AddRemoveRowColumn(Image imgNormal, Image imgHot, Func<string> getRemoveRowToolTipText, Action<int> removeRowAction) {
