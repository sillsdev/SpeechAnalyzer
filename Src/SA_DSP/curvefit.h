...
enum ECURVE_TYPE {PARABOLIC = 3};  // type must be set to number of polynomial coefficients

class CCurveFitting {
public:
    static char * Copyright(void);
    static float Version(void);
    static dspError_t CreateObject(CCurveFitting ** ppCurveFitter, ECURVE_TYPE CurveType);
    ...
    ~CCurveFitting();
private:
    CCurveFitting(ECURVE_TYPE CurveType, float * pCoeff);
    short       m_nCurveType;
    float   *   m_coeff;
    float * m_pData;
};
