...
class multitext : public lift_base {
public:
...
class field_definition : public lift_base {
public:
    ...
    void load(Element * in) {
        ...
        load_attribute(option_range,in);
        load_attribute(writing_system,in);
        load_element(label,in);
        load_element(description,in);
    };
    ...
    Element * store() {
        ...
        store_attribute(option_range,out);
        store_attribute(writing_system,out);
        store_element(label,out);
        store_element(description,out);
        return out;
    };
    ...
    optional<wstring> type;
    optional<key> option_range;
    optional<wstring> writing_system;
    // elements
    optional<multitext> label;
    optional<multitext> description;
};

class fields : public lift_base {
public:
    fields(LPCTSTR name) :
        lift_base(name),
        field_definition(L"field-definition") {
    };

    void load(Element * in) {
        expect(in,name);
        load_element(field_definition,in);
    };

    Element * store() {
        Element * out = new Element(name);
        store_element(field_definition,out);
        return out;
    };

    // attributes
    // elements
    zero_more<field_definition> field_definition;
};
