...
public:
	...
	bool operator!=(const text & right) const {
		if (pcdata.compare(right.pcdata)!=0) return true;
		return false;
	}
...
public:
	...
	bool operator==(const multitext & right) {
		if (form!=right.form) return false;
		if (trait!=right.trait) return false;
		return true;
	}
...
public:
	...
	bool operator==( const annotation & right) const {
		...
		if (who!=right.who) return false;
		if (when!=right.when) return false;
		return true;
	};
...
public:
	...
	bool operator==( const form & right) const {
		...
		if (text!=right.text) return false;
		if (annotation!=right.annotation) return false;
		return true;
	}
...
public:
	...
	void apply_lift_ranges( lift_range_map & range_map) {
		...
		if (!href.has_entry()) return;
		// this href is not in this map
		if (range_map.find( href.get()) == range_map.end()) return;
		lift_ranges & lift_ranges = range_map[href.get()];
		// there is no range data available
		if (!lift_ranges.range.has_entries()) return;
...
public:
	...
	lift_ranges export_lift_ranges( LPCTSTR filename) {
		lift_ranges result;
		if (!range.has_entries()) return result;
...
public:
	...
	lift_ranges export_lift_ranges( LPCTSTR filename) {
		if (!ranges.has_entry()) return lift_ranges();
		return ranges.get().export_lift_ranges(filename);
	}

	void apply_lift_ranges( lift_range_map & range_map) {
		if (!ranges.has_entry()) return;
...
public:
	...
	lift_range_map get_external_range_refs() {
		...
		if (!header.has_entry()) return hrefs;
		if (!header.get().ranges.has_entry()) return hrefs;
	...
	lift_ranges export_lift_ranges( LPCTSTR filename) {
		if (!header.has_entry()) lift_ranges();
		return header.get().export_lift_ranges(filename);
	}

	void apply_lift_ranges( lift_range_map & range_map) {
		if (!header.has_entry()) return;
