...
const int ClefRange[5][2] =
{
	...
	((NAME_D*NUMBER_OF_ACCIDENTALS)+DOUBLE_SHARP+OCTAVE_6)}, // TREBLE
	{((NAME_G*NUMBER_OF_ACCIDENTALS)+DOUBLE_FLAT+OCTAVE_2),
	((NAME_D*NUMBER_OF_ACCIDENTALS)+DOUBLE_SHARP+OCTAVE_5)}, // TREBLE-8VB
	{((NAME_B*NUMBER_OF_ACCIDENTALS)+DOUBLE_FLAT+OCTAVE_1),
	((NAME_F*NUMBER_OF_ACCIDENTALS)+DOUBLE_SHARP+OCTAVE_4)}, // BASS
	{((NAME_B*NUMBER_OF_ACCIDENTALS)+DOUBLE_FLAT+OCTAVE_0),
	((NAME_F*NUMBER_OF_ACCIDENTALS)+DOUBLE_SHARP+OCTAVE_3)}  // BASS-8VB
};
...
void Note::CalcValue(int Clef)
{
	...
	else if (Name>='A'&&Name<='G')
	{
		switch(Accidental)
		{
		case '$': Value = DOTTED_SHARP; break;
		case '#': Value = SHARP; break;
		case '+': Value = HALF_SHARP; break;
		case '=': Value = NATURAL; break;
		case '-': Value = HALF_FLAT; break;
		case '@': Value = FLAT; break;
		case '!': Value = DOTTED_FLAT; break;
	...
	case 'w': // whole note
		...
		break;
	case 'h': // half note
		if (Value==-1) DisplayNote=DISPLAY_HALF_REST;
		else if (Value<ClefRange[Clef][0])
		...
		break;
	case 'q': // quarter note
		if (Value==-1) DisplayNote=DISPLAY_QUARTER_REST;
		else if (Value<ClefRange[Clef][0])
		...
		break;
	case 'i': // eighth note
		if (Value==-1) DisplayNote=DISPLAY_EIGHTH_REST;
		else if (Value<ClefRange[Clef][0])
	...
	case 's': // sixteenth note
		if (Value==-1) DisplayNote=DISPLAY_SIXTEENTH_REST;
		else if (Value<ClefRange[Clef][0])
		...
		break;
	case 'z': // grace note
		if (Value==-1) DisplayNote=DISPLAY_GRACE_REST;
		else if (Value<ClefRange[Clef][0])
...
{
	if ((c>='a' && c<= 'g')||c=='r') c -= 'a' - 'A';
	else if (c>='H' && c<='Z' && c!='R') c += 'a' - 'A';
...
{
	if (Clef<0||Clef>5) return FALSE;
	if (Value<0) return TRUE; // rest
	if (Value<ClefRange[Clef][0]) return FALSE;
	if (Value>ClefRange[Clef][1]) return FALSE;
	return TRUE;
}//Note::InClef
...
unsigned Note::MIDINote()
{
	...
	switch(Name)
	{
	case 'A': cNote += 9; break;
	case 'B': cNote += 11; break;
	case 'C': cNote += 0; break;
	case 'D': cNote += 2; break;
	case 'E': cNote += 4; break;
	case 'F': cNote += 5; break;
	case 'G': cNote += 7; break;
...
unsigned long Note::MIDIDuration()
{
	...
	{
	case 'w': l = 4 * TICKS_PER_Q; break;
	case 'h': l = 2 * TICKS_PER_Q; break;
	case 'q': l = TICKS_PER_Q; break;
	case 'i': l = TICKS_PER_Q / 2; break;
	case 's': l = TICKS_PER_Q / 4; break;
	case 'z': l = TICKS_PER_Q / 8; break;
...
{
	if (!MelodySize) return (Clef=1); // NOTHING TO DISPLAY (DEFAULT:TREBLE)
...
WCHAR *PartWindowData::PartEncode(unsigned Begin,unsigned End)
{
	...
	if (End>MelodySize) End = MelodySize-1;
...
{
	if (End>MelodySize) End = MelodySize;
	...
	else aString = new char[aStringSize];
	...
	for (i=0;i<(End-Begin);++i)
	{
		...
		{
case 0:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(FIELD)"); break;
case ANNOT_BAR:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(-BAR-)"); break;
case ANNOT_THICK_BAR:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(=BAR=)"); break;
case ANNOT_PHRASE:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(\'PHR\')"); break;
case ANNOT_PHRASE2:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(\"PHR\")"); break;
case ANNOT_SPACE:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(SPACE)"); break;
default:
	strcpy_s(&aString[i*7+aStringExtra],aStringSize+aStringExtra-(i*7+aStringExtra),"(??" "??" "?)"); break;
		}
	}
...
int PartWindowData::Paint()
{
	...
	else SelectObject(ps.hdc,hFont);

	// clear frame
	FillRect(ps.hdc,&ps.rcPaint,(HBRUSH)(COLOR_WINDOW+1));
	r.left+=2;r.top+=2;r.right-=4;r.bottom-=4;
	...
	if (Sequence)
	{
		...
		switch(Clef)
		{
		case 1: *FullSequence = DISPLAY_TREBLE; break;
		case 2: *FullSequence = DISPLAY_TREBLE_8VB; break;
		case 3: *FullSequence = DISPLAY_BASS; break;
		case 0: *FullSequence = DISPLAY_TREBLE_8VA; break;
		case 4: *FullSequence = DISPLAY_BASS_8VB; break;
		}
	...
	else // quick draw empty staff
	{
		...
		{
		case 3: // BASS
			*EmptyStaff = DISPLAY_BASS; break;
		case 0: // TREBLE 8VA
			*EmptyStaff = DISPLAY_TREBLE_8VA; break;
		case 2: // TREBLE 8VB
			*EmptyStaff = DISPLAY_TREBLE_8VB; break;
		case 4: // BASS 8VB
			*EmptyStaff = DISPLAY_BASS_8VB; break;
		}
	...
	if (hPlayButton) InvalidateRgn(hPlayButton,NULL,FALSE);
	if (hPauseButton) InvalidateRgn(hPauseButton,NULL,FALSE);
	if (hStopButton) InvalidateRgn(hStopButton,NULL,FALSE);
	if (hLoopButton) InvalidateRgn(hLoopButton,NULL,FALSE);
	EndPaint(hMyself,&ps);
	return 0;
...
unsigned PartWindowData::Position(unsigned Pixels)
{
	...
	switch(Clef)
	{
	case 0: Left=Right=FontCharWidth[DISPLAY_TREBLE_8VA]; break;
	case 1: Left=Right=FontCharWidth[DISPLAY_TREBLE]; break;
	case 2: Left=Right=FontCharWidth[DISPLAY_TREBLE_8VB]; break;
...
void PartWindowData::AutoScrollRight(void)
{
	...
	switch(Clef)
	{
	case 0: ClefWidth = FontCharWidth[DISPLAY_TREBLE_8VA]; break;
	case 1: ClefWidth = FontCharWidth[DISPLAY_TREBLE]; break;
	case 2: ClefWidth = FontCharWidth[DISPLAY_TREBLE_8VB]; break;
	case 3: ClefWidth = FontCharWidth[DISPLAY_BASS]; break;
	case 4: ClefWidth = FontCharWidth[DISPLAY_BASS_8VB]; break;
	}
	unsigned Width = 0;
		...
		case 0xF: // sysex/meta events
			...
			case 0xFF: // meta event
				...
				case 0x2F: // end of track
					if (MelodySize)//!!
					{ if (!bMusiqueFile) SetClef(WhichClef());
					InvalidateRgn(hMyself,NULL,0);
					return 0;
					}
...
BOOL PartWindowData::RTFOut(const char *filename)
{
	...
	if (outfile)
	{
		...
		switch(Clef)
		{
		case 1: _ltoa_s(DISPLAY_TREBLE,aString,_countof(aString),10); break;
		case 2: _ltoa_s(DISPLAY_TREBLE_8VB,aString,_countof(aString),10); break;
		case 3: _ltoa_s(DISPLAY_BASS,aString,_countof(aString),10); break;
		case 0: _ltoa_s(DISPLAY_TREBLE_8VA,aString,_countof(aString),10); break;
		case 4: _ltoa_s(DISPLAY_BASS_8VB,aString,_countof(aString),10); break;
		}
		outfile.write("\\uc0",4);
		outfile.put('\\');
		outfile.put('u');
			...
			case VK_LEFT:
				...
				else
				{
					if (pwd->SelectEnd &&
						...
						--pwd->SelectEnd;
					if (!pwd->Selecting) pwd->SelectBegin = pwd->SelectEnd;
	...
	case WM_COPY:
		{
			...
			if (pwd)
			{
					...
					(!pwd->SelectMin() && pwd->SelectMax()==pwd->MelodySize))
				{
					...
					switch(pwd->Clef)
					{
					case 1: _ltoa_s(DISPLAY_TREBLE,&RTFText[i],len-i,10); break;
					case 2: _ltoa_s(DISPLAY_TREBLE_8VB,&RTFText[i],len-i,10); break;
					case 3: _ltoa_s(DISPLAY_BASS,&RTFText[i],len-i,10); break;
					case 0: _ltoa_s(DISPLAY_TREBLE_8VA,&RTFText[i],len-i,10); break;
					case 4: _ltoa_s(DISPLAY_BASS_8VB,&RTFText[i],len-i,10); break;
					}
					while(RTFText[i]) ++i;
				...
				if (Text)
				{
					if (*RTFText && *Text) Clip.SetTextRTF(RTFText,Text);
					delete [] RTFText;
					delete Text;
				}
			}
				...
				case SB_PAGELEFT:
					{
						...
						if (pwd->SelectEnd) --pwd->SelectEnd;
						pwd->ViewBegin = 0;
						pwd->AutoScrollRight();
						pwd->SelectEnd = j;
						InvalidateRgn(hWnd,NULL,FALSE);
	...
	case WM_COMMAND:
		{
			...
			else return SendMessage(hWnd,(UINT)wParam,0,0L);
		}
	}
...
static DWORD PlayMidiFile(char *lpszMIDIFileName, HWND hWndNotify)
{
	if (wDeviceID) mciSendCommand(wDeviceID, MCI_CLOSE, 0, NULL);
