...
static char* NoteNum2Name(UBYTE byMIDINumber, char* sMusique);
...
char *PartWindowData::GetString(unsigned Begin, unsigned End)
{
  ...
  if (!Begin && End==MelodySize)
  {
    ...
    strcpy(aString,"(CLEFx)(VXxxx)(Q=xxx)");
  ...
  for (i=0;i<(End-Begin);++i)
  {
    ...
    {
      case 0:
        strcpy(&aString[i*7+aStringExtra],"(FIELD)"); break;
      case ANNOT_BAR:
        strcpy(&aString[i*7+aStringExtra],"(-BAR-)"); break;
      case ANNOT_THICK_BAR:
        strcpy(&aString[i*7+aStringExtra],"(=BAR=)"); break;
      case ANNOT_PHRASE:
        strcpy(&aString[i*7+aStringExtra],"(\'PHR\')"); break;
      case ANNOT_PHRASE2:
        strcpy(&aString[i*7+aStringExtra],"(\"PHR\")"); break;
      case ANNOT_SPACE:
        strcpy(&aString[i*7+aStringExtra],"(SPACE)"); break;
      default:
        strcpy(&aString[i*7+aStringExtra],"(??" "??" "?)"); break;
    }
  }
...
int PartWindowData::Paint()
{
  ...
  if (Sequence)
  {
    ...
    wcscpy(FullSequence+1,Sequence);
...
DWORD PartWindowData::Ticks2Dur(DWORD dwNoteTicks, DWORD dwTolerance, char* sMusique)
{
  ...
  strcat(sMusique, sNoteDur[nNoteIndex]);
  if ((dwResult) && (!bRest))
    strcat(sMusique, "_");
  else
    strcat(sMusique, " ");
      ...
      if ((byEventTrack == byEventTrackFirst) /*&& !bySamaFlag*/)
      {
        if ((byEventCommand == 0x90) && (byNoteVel)) // Note On message
        {
          if (dwNoteDeltaTime > dwNoteTolerance) // there is a rest here
          {
            ...
            {
              strcpy(sMusique, "R=3*");
              dwNoteResidue = Ticks2Dur(dwNoteDeltaTime, dwNoteTolerance, sMusique);
        ...
        else // Note Off message
        {
          if (bNoteOn && dwNoteDeltaTime)
          {
            if(!bySamaFlag)
              ...
              {
                *sMusique = '\0';
                NoteNum2Name(byNoteNum, sMusique); // name, accidental, octave
                strcat(sMusique, "*"); // no onset glide
                dwNoteResidue = Ticks2Dur(dwNoteDeltaTime, dwNoteTolerance, sMusique);
                dwNoteDeltaTime = dwNoteResidue;
                ReplaceString(sMusique,MelodySize,MelodySize);
              } while (dwNoteResidue);
    ...
    case WM_APP + 2: // play selection
      {
        ...
        if (pwd)
        {
          ...
          if(strlen(lpszTempFileName)>3)
            strcpy(lpszTempFileName + strlen(lpszTempFileName) - 3,"mid");
    ...
    case WM_TIMER:  // Play feedback timer
      {
        ...
        if (pwd)
        {
          ...
          double dHere = 0;
		  unsigned i;
          for(i=pwd->m_sPlay.nMin; i < pwd->m_sPlay.nMax; i++)
          {
            dHere +=  pwd->Melody[i]->MIDIDuration();
    ...
    case WM_APP + 3: // read file
      {
        ...
        if (pwd)
        {
          ...
          if (lParam && *((char*)lParam)) strcpy(szFile,(char*)lParam);
          else szFile[0] = '\0';
