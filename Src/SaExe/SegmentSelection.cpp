...
/***************************************************************************/
BOOL CSegmentSelection::SelectFromPosition(CSaView * pView, int nSegmentIndex, DWORD dwPosition, int nMode)
{
    // Get original selection for return test
    Update(pView);
    int nAnnotationIndex = m_nAnnotationIndex;
    ...
    if ((nMaster != nSegmentIndex) && (pSegment->GetSelection() == -1))
    {
        ...
        if (nSelection != -1)   //Set up virtual selection
        {
            ...
            if (m_bVirtual)
            {
                RefreshAnnotation(pView, m_nAnnotationIndex);
            }
            m_bVirtual = TRUE;
            m_nAnnotationIndex = nSegmentIndex;
    ...
    Update(pView);
    return !((nAnnotationIndex == m_nAnnotationIndex)&&(dwStart == m_dwStart));
}
...
// a position. Returns TRUE if selection changed else FALSE
/***************************************************************************/
BOOL CSegmentSelection::SelectFromStopPosition(CSaView * pView, int nSegmentIndex, DWORD dwStop, int nMode)
{
    // Get original selection for return test
    Update(pView);
    int nAnnotationIndex = m_nAnnotationIndex;
...
void CSegmentSelection::Update(CSaView * pView, BOOL bClearVirtual)
{
    if (pView->IsAnyAnnotationSelected())
    {
        ...
        m_bVirtual = FALSE;
        m_nAnnotationIndex = pView->FindSelectedAnnotationIndex();

        CSegment * pSegment = pView->GetDocument()->GetSegment(m_nAnnotationIndex);
...
CString CSegmentSelection::GetSelectedAnnotationString( CSaView * pView, BOOL bRemoveDelimiter) const
{
    ...
    if (bRemoveDelimiter)
    {
        if ((ret[0] == SEGMENT_DEFAULT_CHAR) && (m_nAnnotationIndex != GLOSS))
        {
            return ret.Mid(1);
        }

        if (((ret[0] == TEXT_DELIMITER)||(ret[0] == WORD_DELIMITER)) && (m_nAnnotationIndex == GLOSS))
        {
            return ret.Mid(1);
        }
    }
...
{
    if (m_nAnnotationIndex == -1)
    {
        return FALSE;
    }

    int nIndex = m_nAnnotationIndex;
    ...
    if (m_bVirtual)
    {
        ...
        {
            ((CDependentSegment *) pSegment)->Add(pView->GetDocument(), m_dwStart, szString, FALSE, bCheck);
        }
        else
        {
