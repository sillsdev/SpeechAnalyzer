...
/***************************************************************************/
char* CPlotStaff::NoteNum2Name(double dMIDINumber, char* sMusique, BOOL bHalfSharpFlat)
{
  // $#+=-@!
  ...
  else
  {
    ...
    // construct note name
    strcpy(sMusique, &sNoteNamesHalfSharp[nInterval][0]);
  }
...
double CPlotStaff::QNotes2Dur(double dNoteQtrNotes, char *sDuration)
{
  ...
  strcpy(sDuration, sNoteDur[nNoteIndex]);
...
double CPlotStaff::Note2String(double dNoteQtrNotes, double dMIDINumber, CString & sMusique, double dNoteTol, BOOL bHalfFlatSharp)
{
   ...
   if (!bRest)
   {
     (void)NoteNum2Name(dMIDINumber, sNote, bHalfFlatSharp); // name, accidental, octave
     strcat(sNote, "*");
   }
   else
   {
   ...
   while (dNoteQtrNotes >= dNoteTol)
   {
     ...
     dNoteQtrNotes = QNotes2Dur(dNoteQtrNotes, sDur);
