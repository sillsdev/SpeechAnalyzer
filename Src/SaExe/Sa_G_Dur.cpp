...
void CPlotDurations::OnDraw(CDC * pDC, CRect rWnd, CRect rClip, CSaView * pView)
{
  ...
  if (nResult != PROCESS_CANCELED)
  {
    ...
    DurationsPaint(0, pDurations->GetMaxDuration(), pDC,
      rWnd, rClip, pDurations); // do standard data paint
  }

  else PlotPaintFinish(pDC, rWnd, rClip);
}
...
void CPlotDurations::DurationsPaint(DWORD /*dwMinDuration*/, DWORD dwMaxDuration, CDC* pDC,
                                    ...
                                    CDataProcess* pProcess, int nFlags)
{
  ...
  if (!pSegment->IsEmpty()) // string is not empty
  {
    ...
    {
      if ((DWORD)pSegment->GetStop(nSegStart) >= dwDataStart) break;  // quit at segment just beyond start of clip region
