...
void CDlgVocalTract::OnGetAll() 
{
  ...
  CSaDoc* pDoc = (CSaDoc*)pApp->IsFileOpened(m_szSourceFilename);
  {
    ...
    if (nResult == PROCESS_ERROR || nResult == PROCESS_CANCELED) return;
...
void CDlgVocalTract::OnGetSegments(CFlexEditGrid &cGrid)
{
  ...
  if (wSmpSize == 1) Signal.SmpDataFmt = PCM_UBYTE;   //samples are unsigned 8 bit
  else Signal.SmpDataFmt = PCM_2SSHORT;               //samples are 2's complement 16 bit
...
BOOL CDlgVocalTract::SynthesizeDataChunk(HMMIO hmmioFile, PCMWAVEFORMAT pcm, CIpaVTCharVector &cChars)
{
	...
	for(;i < cChars.size();i++)
  {
    ...
    if(artificialStimulus && cChars[i].m_stimulus.VHX)
    {
      ...
      {
        artificialStimulusAspiration.Forward().Tick(randOneRms());
        artificialStimulusVoice.Forward().Tick(impulse(dImpulsePhase - j*dImpulsePhaseChange, nHarmonics));
      }
    }
    ...
    while(data.size() < nSamples)
    {
      ...
      if(artificialStimulus)
      {
        ...
        if(AH)
        {
          stimulus += 0.1*AH*artificialStimulusAspiration.Forward().Tick(randOneRms());
          
          if(dAspOpen < data.size() && cChars[i].m_stimulus.Pitch)
          {
            stimulus += 1.4*AH*artificialStimulusAspiration.Forward().Tick(randOneRms());
          }
        }
        ...
        if(dVoice <= data.size())
        {
          ...
          if(dVoicePitch > 0)
          {
            double dImpulse = artificialStimulusVoice.Forward().Tick(impulse(dImpulsePhase, nHarmonics));
