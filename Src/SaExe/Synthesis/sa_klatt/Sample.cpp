...
Float CKSynth::next_sample()
{
  ...
  if (!synth.parallel_only_flag)
  {
    ...
    switch (spkrdef.NF)
    {
    case 8:    casc = formant_8_cascade.AdvanceResonator(casc);
    case 7:    casc = formant_7_cascade.AdvanceResonator(casc);
    case 6:    casc = formant_6_cascade.AdvanceResonator(casc);
  ...
  case 1:
  case 2:
  case 3:	 output = out[spkrdef.OS] + 0.99 * synth.integrator;
    synth.integrator = output;
    break;

  default:  output = out[spkrdef.OS];
    break;
  }
  return output;
}
