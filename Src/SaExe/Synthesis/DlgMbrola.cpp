...
void CDlgMbrola::OnMbrolaGet()
{
  ...
  if (m_bGetPitch) // formants need pitch info
  {
    ...
    if (nResult == PROCESS_ERROR) m_bGetPitch = FALSE;
  ...
  while(nIndex != -1)
  {
    ...
    if (m_bGetIPA) m_cGrid.SetTextMatrix(rowIpa,column,szString);
  ...
  if (nGender > 1) nGender = 1;   // force to female since no dictionary exists for child
    ...
    if(!IPA.IsEmpty())
    {
      ...
      { 
		...
		if (bCombine) 
        {
		  ...
		  pitch /= duration;
		  if (pitch > 0) pitchText.Format(_T("%.5g"), pitch);
        }
      }
...
void CDlgMbrola::OnMbrolaSynthesize()
{
  ...
  if (lError != -1) lError = mmioWrite(hmmioFile, (HPSTR)&SynthesisFmtParm.wChannels, sizeof(WORD));
  if (lError != -1) lError = mmioWrite(hmmioFile, (HPSTR)&SynthesisFmtParm.dwSamplesPerSec, sizeof(DWORD));
  if (lError != -1) lError = mmioWrite(hmmioFile, (HPSTR)&SynthesisFmtParm.dwAvgBytesPerSec, sizeof(DWORD));
  if (lError != -1) lError = mmioWrite(hmmioFile, (HPSTR)&SynthesisFmtParm.wBlockAlign, sizeof(WORD));
  if (lError != -1) lError = mmioWrite(hmmioFile, (HPSTR)&SynthesisFmtParm.wBitsPerSample, sizeof(WORD));
  if (lError == -1)
  {
...
void CDlgMbrola::OnDropdownSource() 
{
  ...
  while (position != NULL)
  {
    ...
    if(!path.IsEmpty()) m_cSource.AddString(path);
  }
