...
public:
  ...
  BOOL IsOpen() {return m_hInDev ? TRUE : FALSE;}
  BOOL AttachHighPassFilter(USHORT wSmpRate);
  CHighPassFilter70* GetHighPassFilter() {return m_pHighPassFilter;}
  void DetachHighPassFilter() {delete m_pHighPassFilter; m_pHighPassFilter = NULL;}
  ...
  int GetPreparedBuffers() const { return m_nBuffers;}
...
public:
  ...
  void Reset();
  BOOL IsOpen() {return m_hOutDev ? TRUE : FALSE;}
  BOOL ShowMixer(BOOL bShow = TRUE);
  BOOL ConnectMixer(CWnd *pCallback);
...
public:
  ...
  void Stop();
  WAVEFORMATEX *GetFormat() {return (WAVEFORMATEX*)m_pFmtParm;}
  int GetActiveBlock() {return m_nActiveBlock;} // returns the last block ID
  int GetNextBlock() {return m_nNextBlock;} // returns the last block ID
  ...
  UINT GetVolume(BOOL *bResult = NULL) {return m_pOutDev->GetVolume(bResult);}  // get playback volume
  void SetVolume(UINT nVolume) {m_pOutDev->SetVolume(nVolume);} // set playback volume
  void SetSpeed(UINT nSpeed) {m_nSpeed = nSpeed;} // set playback speed
  void SetMaxLevel(UINT nMaxLevel) {m_nMaxLevel = nMaxLevel;}  // set peak magnitude
  CWaveOutDevice* GetOutDevice() {return m_pOutDev;}
  CWaveInDevice* GetInDevice() {return m_pInDev;}
};
