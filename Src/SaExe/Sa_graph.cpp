...
void CGraphWnd::Clear(void)
{
	...
	if (m_pXScale) delete m_pXScale;

	for (int nLoop = 0; nLoop < ANNOT_WND_NUMBER; nLoop++)
	{
		if (m_apAnnWnd[nLoop]) delete m_apAnnWnd[nLoop];
	}
}
...
void CGraphWnd::ShowLegend(BOOL bShow, BOOL bRedraw)
{
	...
	if (!bRecursion)
	{
		if (IsPlotID(IDD_MELOGRAM))
		{
			...
			bRecursion = TRUE;
			if (pGraph) pGraph->ShowLegend(m_bLegend, bRedraw);
			bRecursion = FALSE;
		}
	...
	if (bRedraw) ResizeGraph(TRUE, TRUE); // repaint whole graph window
}
...
void CGraphWnd::ResizeGraph(BOOL bEntire, BOOL bLegend, BOOL bGraph)
{
	if (bEntire)
	{
		...
		if (IsPlotID(IDD_MELOGRAM))
		{
			...
			if (pGraph) pGraph->ResizeGraph(TRUE, pGraph->HaveLegend());
		}
	}
...
void CGraphWnd::RedrawGraph(BOOL bEntire, BOOL bLegend, BOOL bGraph)
{
	...
	if(bEntire)
	{
		...
		if (IsPlotID(IDD_MELOGRAM))
		{
			CSaView* pView = (CSaView*)GetParent();
			CGraphWnd* pGraph = pView->GraphIDtoPtr(IDD_TWC);
			if (pGraph) pGraph->RedrawGraph(TRUE, pGraph->HaveLegend());
		}
	}
}
...
void CGraphWnd::UpdateStatusBar(DWORD dwStartCursor, DWORD dwStopCursor, BOOL bForceUpdate)
{
	...
	if ((m_bFocus) && (pView->ViewIsActive()))
	{
		...
		if (HavePrivateCursor())
		{
			...
			case IDD_SPECTRUM:
				...
				if (pSpectrum->IsDataReady())
				{
					...
					if (nPitchMode == HERTZ)
						_stprintf(szText, _T("    %.1f Hz"), fData);
					else
						_stprintf(szText, _T("    %.1f st"), GetSemitone(fData));
					...
					if (pSpectrum->GetSpectralData(0).Smooth != (float)UNDEFINED_DATA)
					{
						...
						bRes = FALSE;
					_stprintf(szText, _T("Cep:"));
					if (bRes && pSpectrumParm->bShowCepSpectrum)
					{
						if (pSpectrumParm->nScaleMode)
							_stprintf(szText, _T("Cep: %5.1f"), fData);
						else
							_stprintf(szText, _T("Cep: %5.1f dB"), fData);
					}
					...
					if (pSpectrum->GetSpectralData(0).Raw != (float)UNDEFINED_DATA)
					{
						...
						bRes = FALSE;
					_stprintf(szText, _T("Raw:"));
					if (bRes)
					{
						if (pSpectrumParm->nScaleMode)
							_stprintf(szText, _T("Raw: %5.1f"), fData);
						else
							_stprintf(szText, _T("Raw: %5.1f dB"), fData);
					}
					...
					if (pSpectrum->GetSpectralData(0).Lpc != (float)UNDEFINED_DATA)
					{
						...
						if (pSpectrumParm->nScaleMode == DB)
						{
							...
							else LpcRef = pSpectrum->GetSpectralPowerRange().fdBRef;
						...
						bRes = FALSE;
					_stprintf(szText, _T("LPC:"));
					if (bRes)
					{
						if (pSpectrumParm->nScaleMode)
							_stprintf(szText, _T("LPC: %5.1f"), fData);
						else
							_stprintf(szText, _T("LPC: %5.1f dB"), fData);
					}
			...
			case IDD_TWC:
				...
				if (pTWC->IsDataReady())
				{
					...
					// prepare pitch pane
					// DDO - 08/15/00 _stprintf(szText, "%7.0fHz (%3s)", SemitoneToFrequency(fSemitone), Semitone2Name(bySemitone, sNoteName));
					...
					fFrequency += 0.5 * pow(10.F, -nPrecision);
		...
		case IDD_CHPITCH:
			...
			{
			if (nPitchMode == HERTZ)
			_stprintf(szText, "    %.1f Hz", fData);
			else
			_stprintf(szText, "    %.1f st", GetSemitone(fData));
			}
			}
			}
		...
		if (bShowPosition)
		{
			...
			else
			{
				if (nPositionMode == SAMPLES)
					_stprintf(szText, _T("       %.0f"), (double)(dwStartCursor / nSmpSize));
				else
					_stprintf(szText, _T("       %.0f"), (double)dwStartCursor);
			}
			// write to position pane
	...
	else return; // this graph has not focus
}
