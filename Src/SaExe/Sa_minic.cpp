...
/***************************************************************************/
BOOL CMiniCaptionWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle,
...
BOOL CMiniCaptionWnd::PreCreateWindow(CREATESTRUCT& cs)
{
  ...
  else // normal (thick frame) window border
  {
    ...
    else cs.style = WS_VISIBLE | WS_CHILD | WS_CLIPSIBLINGS | WS_THICKFRAME | WS_BORDER | WS_SYSMENU;
  }
  return bRet;
}
...

{
  ...
  if (bClientCoords) // client coordinates
  {
    prcNC->top = 0;
...
UINT CMiniCaptionWnd::OnNcHitTest(CPoint point)
{
  ...
  if (rcNC.PtInRect(point))
  {
    ...
    if (rcSysMenu.PtInRect(point)) return HTSYSMENU; // system menu area
    if (rcRightmost.PtInRect(point)) return HTCLOSE; // rightmost button
    if (rcRightTwo.PtInRect(point)) // minimize for Win95, caption for Win31
      return HTMINBUTTON;
    if (rcRightOne.PtInRect(point)) return HTMAXBUTTON; // second right button
    // in minimized state and win95 caption has system menu function
    if (wpl.showCmd == SW_SHOWMINIMIZED) return HTCAPTION;
...
void CMiniCaptionWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS * lpncsp)
{
  ...
  if ((m_nCaption == NoneThin) || (m_nCaption == Normal)) return; // standard
  ...
  if (wpl.showCmd != SW_SHOWMAXIMIZED) // window is not maximized
...
void CMiniCaptionWnd::OnNcPaint()
{
  ...
  CWnd::OnNcPaint();
  if (m_nCaption == NoneThin) return; // nothing else to draw
  CWindowDC dc(this);
...
void CMiniCaptionWnd::OnNcDraw(CDC * pDC, const CRect & printRect, BOOL bPrintDoubleHeight)
{
  ...
  wpl.length = sizeof(WINDOWPLACEMENT);
  GetWindowPlacement(&wpl);
  CalcNCAreas(pDC->IsPrinting(),printRect, bPrintDoubleHeight,
    &rcNC, &rcSysMenu, &rcCaption, &rcRightmost, &prcRightTwo, &prcRightOne, &rcFrame);
  if (m_nCaption != Normal)
  {
    ...
    if (m_nCaption == Mini || m_nCaption == MiniWithCaption) // caption and bitmaps needed
    {
      ...
      if (pDC->IsPrinting())
      {
        Darken(textColor); // we need a dark text color because the background
      ...
      if (wpl.showCmd == SW_SHOWMINIMIZED)// Win95 minimized
      {
        ...
        CRect rTemp;
        GetWindowRect(rTemp);
        rcWnd.bottom = rcWnd.top + rTemp.Height() - 2 * m_iySizFrame + 2;
      ...
      else
      {
        ...
        if (m_nCaption == MiniWithCaption) // draw the miniaturized caption.
        {
          ...
          // draw the text (left aligned for Win95, centered in Win31
          iTmp = pDC->DrawText(szText, -1, rcWnd, DT_VCENTER | DT_SINGLELINE | DT_LEFT);
          pDC->SelectObject(pOldFont); // allows for proper restoration
        }
      }
      ...
      if (!pDC->IsPrinting())
      {
        ...
        pDC->StretchBlt(rcSysMenu.left, rcSysMenu.top, rcSysMenu.Width(),
          rcSysMenu.Height(), pDisplayMemDC, 0, 0, 32, 32, SRCCOPY);
        ...
        pbmpOld = (CBitmap*)pDisplayMemDC->SelectObject(pbmp);
        pDC->BitBlt(rcRightmost.left, rcRightmost.top, rcRightmost.Width(), rcRightmost.Height(),
        ...
        pbmpOld = (CBitmap*)pDisplayMemDC->SelectObject(pbmp);
        pDC->BitBlt(prcRightOne.left, prcRightOne.top, prcRightOne.Width(), prcRightOne.Height(),
          pDisplayMemDC, 0,0, SRCCOPY);
        delete pDisplayMemDC->SelectObject(pbmpOld); // deletes pbmp
        ...
        pbmpOld = (CBitmap*)pDisplayMemDC->SelectObject(pbmp);
        pDC->BitBlt(prcRightTwo.left, prcRightTwo.top, prcRightTwo.Width(), prcRightTwo.Height(),
          pDisplayMemDC, 0,0, SRCCOPY);
        delete pDisplayMemDC->SelectObject(pbmpOld); // deletes pbmp
      } // end if IsPrinting
    }
  ...
  if (m_nCaption == Normal)
  {
    ...
    rcWnd = rcNC;
    {
      ...
      if (GetSystemMetrics(SM_CYSIZE) >= 19) rcWnd.left--;
      ...
      if (wpl.showCmd != SW_SHOWMINIMIZED) rTempLeft.left++;
      if (GetSystemMetrics(SM_CYSIZE) >= 19) rTempLeft.top--;
      ...
      if (wpl.showCmd != SW_SHOWMINIMIZED) rTemp.right++;
...
void CMiniCaptionWnd::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
  ...
  switch (nHitTest)
  {
    case HTMINBUTTON:
    case HTMAXBUTTON:
    case HTCLOSE:     return; break; // dont't call base class OnNcLButtonDown for this three buttons!
    default: break;
  }
  // get window placement information
  WINDOWPLACEMENT wpl;
  ...
  if ((wpl.showCmd == SW_SHOWMAXIMIZED) && (nHitTest == HTCAPTION)) return;
  CWnd::OnNcLButtonDown(nHitTest, point);
}
...
void CMiniCaptionWnd::OnNcLButtonDblClk(UINT nHitTest, CPoint point)
{
  ...
  if (nHitTest == HTSYSMENU) // system menu dblclk
  {
    ...
    else OnSysCommand(SC_CLOSE, 0L); // close the window
  }
  else
  {
...
void CMiniCaptionWnd::OnNcLButtonUp(UINT nHitTest, CPoint point)
{
  ...
  switch (nHitTest)
  {
    ...
    case HTCLOSE:
      ...
      break;
    default: CWnd::OnNcLButtonDown(nHitTest, point);
  }
}
