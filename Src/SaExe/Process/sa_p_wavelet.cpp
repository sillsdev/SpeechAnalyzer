...
// Constructor
//*  Description   : Creates a new wavelet node
//*  Parameters    : None
//*  Preconditions : None
//*  Postcondtions : None
//*  Returns       : 
...
BOOL CWaveletNode::_DoMRAAnalysisTree(long stride)
{
	...
	mid = lower_freq + ((upper_freq - lower_freq) / 2);

	if ((left_node != NULL)	&& (right_node != NULL))		// both leaves need to be split futher
	{
		// Allocate the memory of the leaves
		
		if (!left_node->SetDataNode(NULL, dwDataSize, lower_freq, mid)) return FALSE;
		if (!right_node->SetDataNode(NULL, dwDataSize, mid, upper_freq)) return FALSE;
		...
		stride *= 2;
		if (!left_node->_DoMRAAnalysisTree(stride)) return FALSE;
		if (!right_node->_DoMRAAnalysisTree(stride)) return FALSE;
...
BOOL CWaveletNode::TransformEnergyTree()
{
	...
	if (left_node != NULL) left_node->TransformEnergyTree();
	if (right_node != NULL) right_node->TransformEnergyTree();
...
BOOL CWaveletNode::TransformLogScalingTree(double high)
{
	...
	if (left_node != NULL) left_node->TransformLogScalingTree(high);
	if (right_node != NULL) right_node->TransformLogScalingTree(high);
...
BOOL CWaveletNode::TransformLogScalingNode(double high)
{
	...
	for(data_index = 0; data_index < dwDataSize; data_index++)
	{
		if (data[data_index] < (.0001 * (double)high)) data[data_index] = long(.0001 * (double)high);
...
BOOL CWaveletNode::TransformSmoothingTree()
{
	...
	if (left_node != NULL) left_node->TransformSmoothingTree();
	if (right_node != NULL) right_node->TransformSmoothingTree();
...
BOOL CWaveletNode::DrawColorBandNode(unsigned char *pBits, CRect *rWnd, long thickness, long y_start, double high, double start, double end)
{
	...
	for (long y = y_start; y < (y_start + thickness); y++)		// Draw "thickness" num of lines
	{
		...
		{	
			...
			data_index += fHScale;
			if (data_index > dwDataSize) break;
...
//************************************************************************** 
// C W A V E L E T  N O D E   H E L P E R  R O U T I N E S
//************************************************************************** 
...
CWaveletNode *CWaveletNode::_GetNode(long level, bool reset)
{
	...
	{
		count++;
		if (count == level) return this;
	}

	if (left_node != NULL) 
	{
		finalNode = left_node->_GetNode(level, false);
		if (count == level) return finalNode;
	}
...
//*									 of the waveform
//*  Parameters    : int **pDataOut (out) - a pointer to the pointer to recieve
//*																					the data
...
BOOL CProcessWavelet::Get_Raw_Data(long **pDataOut, DWORD *dwDataSizeOut, ISaDoc *pDoc)
{
	...
	*pDataOut = (long *)malloc(dwDataSizeBytes * 2);
	...
	while(1)
	{
		...
		// see if we are done
		if (dwDataPosBytes == dwDataSizeBytes) break;
