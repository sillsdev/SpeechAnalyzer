...
/////////////////////////////////////////////////////////////////////////////
// CProcessRaw construction/destruction/creation
						  ...
						  int nLevel)
{
	...
	short int * pProcessedData = (short int *)m_lpData; // pointer to process data
	...
	while (dwDataPos < dwDataSize) // processing loop
	{
		...
		*pProcessedData++ = (short)(nRes * nScale);
		...
		if ((++dwProcessCount == GetProcessBufferSize() / 2) || (dwDataPos >= dwDataSize)) // process buffer is full or processing finished
		{
			...
			try
			{ 
				Write((HPSTR)m_lpData, dwProcessCount * sizeof(short));
			}
			...
			pProcessedData = (short int *)m_lpData; // reset pointer to begin of process data buffer
		}
	}
