...
/***************************************************************************/
CProcessSpectrogram::CProcessSpectrogram(const SpectroParm& cSpectroParm, ISaDoc* pDoc, BOOL bRealTime) 
...
{
  if (m_pSpectroFormants) delete m_pSpectroFormants;
}
...
/***************************************************************************/
void CProcessSpectrogram::SetSpectroParm(const SpectroParm& cSpectroParm)
{
...
/***************************************************************************/
long CProcessSpectrogram::Process(void* pCaller, ISaDoc* pDoc, CSaView *pView, int nWidth, int /*nHeight*/,
...
{   
  ...
  const SpectroParm* pSpectroParm = & GetSpectroParm(); // get pointer to spectrogram parameters
  ...
  // twice wHalfCalcWindow. The wideband version of this calculation has also
  // to be done, because if formants tracks are on, we need to check also on the
  // wideband results, if there is enough data to calculate (spectrogram doesn't
  // do that).
