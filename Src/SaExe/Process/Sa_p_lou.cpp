...
{   
    ...
    short int * pLoudData = (short int *)m_lpData; // pointer to loudness data
    // calculate current processing position
    DWORD dwDataPos, dwLoudStart, dwLoudCount;
    ...
    for (dwLoudCount = 0; dwLoudCount < dwLoudStart; dwLoudCount++)
    { 
    ...
    { 
      ...
      { 
        ...
        if (!pBlockStart) return Exit(PROCESS_ERROR); // error, reading failed
        ...
        if(dOldResult[2] >= 0)
        {
          *pLoudData++ = (short)(sqrt(dResult) + 0.5); // rms magnitude
          ...
          { 
            ...
            pLoudData = (short int *)m_lpData; // reset pointer to begin of loudness data buffer
          }     
        }
        dResult = 0;
      }
    ...
    if (bResult) return MAKELONG(nLevel, nProgress);
    else
    { 
      SetDataInvalid();
...
CProcessSmoothLoudness::~CProcessSmoothLoudness()
{
    ...
    if (m_pSRDfile) delete m_pSRDfile;
...
{
    if (m_pSRDfile) m_pSRDfile->Abort();
    try
...
{   
    if (nOldBlock > nBlock) return pTarget; // old data needed, just give back the buffer pointer
    if (*pnLastDone >= nBlock) return pTarget + GetProcessBufferSize(); // this block is ready in buffer
    *pnLastDone = nBlock;
    ...
    // check end of data
    if ((dwDataPos + nSmoothWidth) >= dwDataSize) return pTarget + GetProcessBufferSize(); // end of data
    // load new raw data block
    HPSTR pBlockStart = pDoc->GetWaveData(dwDataPos, TRUE); // get data
    if (!pBlockStart) return NULL; // reading failed
    if (nOldBlock == -1) // first call
    { // form the moving average seed
      ...
      *lAverage = 0;
      ...
      { short int * lpSource = (short int *)pBlockStart; // cast pointer
        ...
        for (int i = 0; i < nLoopEnd; i++) *lAverage += *lpSource++; // get data
      }
      else // 8 bit data
      { unsigned char * lpSource = (unsigned char *)pBlockStart; // cast pointer
        for (int i = 0; i < nSmoothWidth; i++) nSeed += *lpSource++; // get data
    ...
    { short int * lpSource = (short int *)pBlockStart; // cast pointer
      short int * lpTarget;
      if (nBlock > 0) lpTarget = (short int *)(pTarget + GetProcessBufferSize());
      // CLW 12/3/98
    ...
    { unsigned char * lpSource = (unsigned char *)pBlockStart; // cast pointer
      ...
      if (nBlock > 0) lpTarget = (unsigned char *)(pTarget + GetProcessBufferSize());
      else lpTarget = (unsigned char *)(pTarget + GetProcessBufferSize() + nSmoothWidth + 1);
      do
      { if (dwEnd == dwReloadPos) // new data block has to be reloaded
...
{   
    ...
    if (pUttParm->nMinFreq == 0) pUttParm->nMinFreq = 50; // to prevent 0 divisions 
    ...
    while (dwDataPos < dwDataSize) // processing loop
    { pBlockStart = pDoc->GetWaveData(dwDataPos); // get pointer to data block
      if (!pBlockStart) return Exit(PROCESS_ERROR, hSmoothBlock); // error, reading failed
      ...
      { int nOldBlockNumber = nBlockNumber;
        ...
        if (!pSmoothData) return Exit(PROCESS_ERROR, hSmoothBlock); // error, reading failed
      ...
      { nCalcLoop = 0; // reset loop counter
        ...
        *pLoudData++ = (short)nResult; // save result
        if ((int)nResult > m_nMaxValue) m_nMaxValue = (int)nResult; // save maximum value
        nResult = 0;
        if (nSmpSize == 1) // 8 bit data
        { 
          pSmoothData -= (nCalcDataLength + 4) - nCalcInterval; // reset position in raw data
          if (dwDataPos < dwDataSize) dwDataPos -= (nCalcDataLength + 4) - nCalcInterval;
        }
        else // 16 bit data
        { pSmoothData -= ((nCalcDataLength + 4) - nCalcInterval) * 2; // reset position in raw data
          if (dwDataPos < dwDataSize) dwDataPos -= ((nCalcDataLength + 4) - nCalcInterval) * 2;
        }
        // set progress bar
        SetProgress(nProgress + (int)(100 * dwDataPos / dwDataSize / (DWORD)nLevel));
        if (IsCanceled()) return Exit(PROCESS_CANCELED, hSmoothBlock); // process canceled
    ...
    if (GetDataSize() < 2) return Exit(PROCESS_ERROR, hSmoothBlock); // error, not enough data
...
HPSTR CProcessSmoothLoudness::GetSmoothRawData(DWORD dwOffset, BOOL bBlockBegin)
{
    ...
    { m_bSRDBlockBegin = bBlockBegin;
      if (bBlockBegin) m_dwSRDBufferOffset = dwOffset; // given offset ist first sample in data block
      else m_dwSRDBufferOffset = dwOffset - (dwOffset % GetProcessBufferSize()); // new block offset
