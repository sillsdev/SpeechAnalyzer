...
long CProcessPOA::Process(void* pCaller, ISaDoc* pDoc, DWORD dwStart, DWORD dwStop, int nProgress, int nLevel)
{
    ...
    else Signal.Length = (dwWaveSize - dwStart)/wSmpSize; //partly fills    
    ...
    {
      m_lpData = new char[GetDataSize(sizeof(char))];
      if (!m_lpData)
      {
    ...
    VOCAL_TRACT_MODEL *pVocalTractModel = (VOCAL_TRACT_MODEL *)m_lpData;    
    if (pLpcModel->dMeanEnergy) 
      pVocalTractModel->dErrorRatio = pLpcModel->dMeanSqPredError / pLpcModel->dMeanEnergy;
    else pVocalTractModel->dErrorRatio = (double)UNDEFINED_DATA;
