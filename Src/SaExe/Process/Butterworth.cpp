	...
	if (!m_bReverse)
	{
		...
		while (dwDataPos < dwDataSize)
		{
			...
			while (dwDataPos < dwBlockEnd)
			{
				...
				// process data
				nData = round(Forward().Tick(double(nData)));
...
{
	Forward() = CZTransform(0, NULL, NULL);
	Reverse() = CZTransform(0, NULL, NULL);
}
...
void CButterworth::ConfigureProcess(double dSampling)
{
	...
	if(m_bReverse)
	{
		...
		{
		case kftHighPass:
			CascadeHighPass(Reverse(), m_nOrder, m_dFrequency*FilterFilterNorm(m_nOrder), dSampling, rTau);
			break;
		case kftLowPass:
			CascadeLowPass(Reverse(), m_nOrder, m_dFrequency/FilterFilterNorm(m_nOrder), dSampling, rTau);
			break;
		case kftBandPass:
			...
			CascadeLowPass(Reverse(), m_nOrder, (m_dFrequency + m_dBandwidth/2.)/FilterFilterNorm(m_nOrder), dSampling, rTau);
			CascadeHighPass(Reverse(), m_nOrder, (m_dFrequency - m_dBandwidth/2.)*FilterFilterNorm(m_nOrder), dSampling, rTau);
			break;
		default:
	...
	{
	case kftHighPass:
		CascadeHighPass(Forward(), m_nOrder, m_dFrequency*FilterFilterNorm(m_nOrder), dSampling, tau);
		CascadeScale(Forward(), m_dScale);
		break;
	case kftLowPass:
		CascadeLowPass(Forward(), m_nOrder, m_dFrequency/FilterFilterNorm(m_nOrder), dSampling, tau);
		CascadeScale(Forward(), m_dScale);
		break;
	case kftBandPass:
		// Assume a low pass followed by a high pass will suffice
		//   We should probably adjust the filter for center frequency gain
		//   But this is good enough for now...
		CascadeLowPass(Forward(), m_nOrder, (m_dFrequency + m_dBandwidth/2.)/FilterFilterNorm(m_nOrder), dSampling, tau);
		CascadeHighPass(Forward(), m_nOrder, (m_dFrequency - m_dBandwidth/2.)*FilterFilterNorm(m_nOrder), dSampling, tau);
		CascadeScale(Forward(), m_dScale);
		break;
	default:
		// null filter... We could assert here
...
CZTransform CHilbert::AllPass(double pole)
{
	...
	return CZTransform(2, numerator, denominator);
}
