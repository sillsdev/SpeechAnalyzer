...
/////////////////////////////////////////////////////////////////////////////
// CPlotSpectrogram helper functions
...
#define SET_RGB(r,g,b) ((PC_NOCOLLAPSE << 24) | RGB(r,g,b)) // PC_NOCOLLAPSE make new entry if possible
  ...
  if (nRasterCaps & RC_PALETTE) nRasterCaps = TRUE;
  else nRasterCaps = FALSE;
  int nNumColors;
  ...
  if (nDesiredPaletteMode != nPaletteMode) // not yet compatible with this context
  {
    ...
    {
      if (!SpectroPalette.ResizePalette(2*nPaletteSize)) return FALSE;
    }
    WORD wColorIndex;
...
/////////////////////////////////////////////////////////////////////////////
// CPlotSpectrogram message handlers
...
void CPlotSpectrogram::populateBmiColors(RGBQUAD *QuadColors, CSaView *pView)
{
  ...
  const SpectroParm* pSpectroParm = &GetSpectrogram(pDoc)->GetSpectroParm();
...
BOOL CPlotSpectrogram::OnDrawSpectrogram(CDC *pDC, CRect rWnd, CRect rClip, CSaView * pView, BOOL bSmooth, BOOL *bAliased)
{
  ...
  BOOL bFormants = pSpectroParm->bShowF1 || pSpectroParm->bShowF2|| 
	               pSpectroParm->bShowF3 || pSpectroParm->bShowF4|| 
				   pSpectroParm->bShowF5andUp;
  ...
  if(bSmooth)
  {
    ...
    delete [] pPowerRight; pPowerRight = NULL;
    delete [] pPowerLeft; pPowerLeft = NULL;
    delete [] pIndex;
    delete [] pBottom;
  }
  else
...
BOOL CPlotSpectrogram::OnDrawFormantTracksFragment(CDC * pDC, CRect rWnd, CRect rClip, CSaView * pView)
{
  ...
  short int nResult = LOWORD(pSpectroFormants->ExtractFormants(pDoc, 0, pDoc->GetDataSize(), 
    pSpectroParm->bSmoothFormantTracks));
  nResult = CheckResult(nResult, pSpectroFormants); // check the process result
  
  if (nResult == PROCESS_ERROR) return FALSE;
  ...
  if (nResult != PROCESS_CANCELED && pFragments->IsDataReady())
  {
    ...
    // calculate scale factors and starting positions
    if (rClip.left > 0) rClip.left--;
    if (rClip.right < rWnd.right) rClip.right++;
    ...
    if (dwFirstFragment > 0) dwFirstFragment--;
    ...
    for (int nFormantIndex = 1; nFormantIndex <= nNumberFormants; nFormantIndex++)
    {
      ...
      {
	     case 1:   if (!pSpectroParm->bShowF1) continue; break;
       case 2:   if (!pSpectroParm->bShowF2) continue; break;
       case 3:   if (!pSpectroParm->bShowF3) continue; break;
       case 4:   if (!pSpectroParm->bShowF4) continue; break;
       default:  if (!pSpectroParm->bShowF5andUp) continue;
      }
      BOOL bPlot = FALSE;
      ...
      for (DWORD dwFragment = dwFirstFragment; dwFragment <= dwLastFragment; dwFragment++)
      {
        ...
        ASSERT(pFormants);
        if (pFormants->F[0] != (double)UNVOICED && pFormants->F[0] != (double)NA && pFormants->F[nFormantIndex] != (double)NA)
        {
          bPlot = TRUE;
          int y = rWnd.bottom - (int)Round((double)pFormants->F[nFormantIndex]/(double)fFormantFactor);
          if (y >= rWnd.top) bOnScreen = TRUE;
          ...
          {
            pDC->LineTo(x, y);
            //pDC->LineTo(nXPix, y);   //!!temporary replacement for line above
            x = min((int)(((double)FragmentParm.dwOffset + (double)FragmentParm.wLength - 1. - (double)fDataStart) / fSamplesPerPix + 0.5), rClip.right);		    
          }
          pDC->LineTo(x, y);
        }
    ...
    if (pSpectroParm->nColor && !pSpectroParm->bFormantColor) pDC->SetROP2(nDrawMode);
  }
...
BOOL CPlotSpectrogram::OnDrawFormantTracksTime(CDC * pDC, CRect rWnd, CRect rClip, CSaView * pView)
{
  ...
  // get pointer to process class and spectrogram parameters
  CFormantTracker* pSpectroFormants = pDoc->GetFormantTracker(); // get pointer to spectrogram object
  const SpectroParm* pSpectroParm = &GetSpectrogram(pDoc)->GetSpectroParm();
...
{
  if (GetSafeHwnd() && IsIconic()) return FALSE; // nothing to draw
  // get pointer to graph, view, document, application and mainframe
  ...
  if (rWnd.Height() <= 0) return FALSE; // nothing to draw
  ...
  else
  {
    ...
    if (nResult != PROCESS_CANCELED)
    {
	  ...
	  BOOL bShowFormantTracks = pSpectroParm->bShowF1 || pSpectroParm->bShowF2|| 
	                            pSpectroParm->bShowF3 || pSpectroParm->bShowF4|| 
				                pSpectroParm->bShowF5andUp;
      ...
      // display aliased data warning
      //if (bAliased)
      //  m_HelperWnd.SetMode(MODE_TEXT | FRAME_POPIN | POS_HLEFT | POS_VTOP, IDS_HELPERWND_ALIASED, &rWnd);
      //else m_HelperWnd.SetMode(MODE_HIDDEN);
    }
  }
  return TRUE;
}
...
void CPlotSpectrogram::OnDraw(CDC * pDC, CRect rWnd, CRect rClip, CSaView * pView)
{
  ...
  SpectroParm cSpectroParm = GetSpectrogram(pDoc)->GetSpectroParm();
...
BOOL CPlotSpectrogram::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
  ...
  BOOL bShowFormantTracks = (pSpectroParm->bShowF1 || pSpectroParm->bShowF2|| 
    pSpectroParm->bShowF3 || pSpectroParm->bShowF4|| 
    pSpectroParm->bShowF5andUp) && GetFormantProcess(pDoc)->AreFormantTracksReady();
...
/***************************************************************************/
void SetMagnify(double , BOOL )
{
  // Do nothing. Spectrogram does not magnify with other graphs.
}
