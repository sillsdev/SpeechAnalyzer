    ...
    { short int nResult = LOWORD(pFormants->Process(this, pView, rWnd.Width(), TRUE)); // process data
      ...
      { if (nResult)
        ...
        { if (rClip.left > 0) dwFormant = (int)((double)(rClip.left - 1) * fSizeFactor); // first formant in this pixel
          ...
          { if (bLog10) nOldY = nCenterPos - (int)((log10((double)nData) - fBase) * fVScale); // logarithmic scale
            else nOldY = nCenterPos - (int)((double)nData * fVScale); // linear scale
          }
        }
        ...
        { dwFormant = (DWORD)((double)nLoop * fSizeFactor);
          ...
          { pDC->MoveTo(nLoop, nCenterPos);
            if (bLog10) pDC->LineTo(nLoop, nCenterPos - (int)((log10((double)nData) - fBase) * fVScale)); // logarithmic scale
            else pDC->LineTo(nLoop, nCenterPos - (int)((double)nData * fVScale)); // linear scale
          }
        }
        pDC->SelectObject(pOldPen);
      }
