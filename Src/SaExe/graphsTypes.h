...
public:
  ...
  const UINT * GetGraphs() const { return m_nGraphID;} // return checked graphs
  void SetGraphs(const UINT *pGraphIDs, BOOL bClear=TRUE);
  static bool IsLayoutGraph(UINT nID)  {return nID != IDD_RECORDING && nID != IDD_TWC && nID != IDD_MAGNITUDE;}
  void InsertAt(UINT nID, int nIndex);
  bool operator==(const CGraphList &compare) const;
  UINT operator[](int nIndex) const {return (nIndex >= 0 && nIndex  < MAX_GRAPHS_NUMBER) ? m_nGraphID[nIndex] : 0;}
...
{
public:
  CGraphConfigurationVector(LPCTSTR szFilename = NULL, int nTaskType = -1) { SetFilename(szFilename); Load(szFilename, nTaskType);}
...
public:
  ...
  void Remove(int nIndex) {erase(begin()+nIndex);}
  CString SetFilename(const LPCTSTR szFilename) { CString szOld(m_szFilename); m_szFilename = szFilename; return szOld;}
  int GetCountPredefinedSets() const {return m_nPredefinedSets;}

  // publicize some vector member functions
  const CGraphConfiguration& operator[](size_type _P) const {return static_cast<const BaseClass*>(this)->operator[](_P);}
  CGraphConfiguration& operator[](size_type _P) {return static_cast<BaseClass*>(this)->operator[](_P);}
  size_type size() const {return static_cast<const BaseClass*>(this)->size();}
  void push_back(const CGraphConfiguration &entry) { static_cast<BaseClass*>(this)->push_back(entry);}
  inline bool IsEqual(const CGraphConfigurationVector &operand2) const {return *this == operand2;}
