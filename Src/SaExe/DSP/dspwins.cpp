...
int32 DspWin::CalcLength(double bandwidth, int32 SmpRate, int32 windowType)
{
...
dspError_t HanningWin::FromLength(HanningWin** Hanning, int32 Length, uint16 SmpRate)
{
  if (!Hanning) return(Code(INVALID_PARM_PTR));
  if (!SmpRate) return(Code(UNSUPP_SMP_RATE));
  if (Length <= 0) return(Code(INVALID_WIN_LEN));
  float *Data = (float *)malloc(Length * sizeof(float));   //Length can be even or odd
  if (!Data) return(Code(OUT_OF_MEMORY));
  *Hanning = new HanningWin(Data,Length,SmpRate);
  if (!*Hanning) return(Code(OUT_OF_MEMORY));
  return(DONE);
}

int32 HanningWin::CalcLength(float Bandwidth, uint16 SmpRate)
{
...
dspError_t HanningWin::FromBandwidth(HanningWin** Hanning, float Bandwidth, uint16 SmpRate)
{
  if (!Hanning) return(Code(INVALID_PARM_PTR));
...
HanningWin::HanningWin(float *Data, int32 Length, uint16 SmpRate)
{
  m_Data = Data;
  m_Length = Length;
...
#define PERCENT_TRANSITION   .10F
  ...
  if (!Kaiser) return(Code(INVALID_PARM_PTR));
  if (SmpRate == 0.F) return(Code(INVALID_SMP_RATE));
  if (Bandwidth == 0.F || Bandwidth >= SmpRate/2.F) return(Code(INVALID_BANDWIDTH));
  if (Gain == 0.F) return(Code(INVALID_GAIN));
  if (Atten == 0.F) return(Code(INVALID_ATTEN));
                     ...
                     float Transition, int32 GroupDelay, float SmpRate)
{
  ...
  if (Atten > 50.F) m_TradeOff = .1102F*(Atten - 8.7F);
  else if (Atten > 21.F)
...
{
  if (i > m_GroupDelay) i = m_Length - 1 - i%m_Length; //modulo to prevent overflow
  if (!m_CoeffSet[i])
  {
    if (i != m_GroupDelay)
    ...
    else m_Coeff[i] = (float)Round((double)m_Gain*(2.*(double)m_Bandwidth/(double)(m_SmpRate)));
    m_CoeffSet[i] = true;
  }
  return(m_Coeff[i]);
}
...
{
  for (int32 i = 0; i < m_Length; i++) CalcCoeff(i);
