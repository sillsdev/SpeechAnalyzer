           ...
           BOOLFLAG bRefine, BOOLFLAG bSort)
{
    ...
    for (j=0; j <= nOrder; j++)
    {
        ComplexCoeff[j]= DeflatedCoeff[j] = Complex((float)Coeff[j], 0.F);
    }
    for (j=nOrder, k=0; j>=1; j--)
    {
        ...
        if (fabs(Root.imag) <= 2.0*EPS*fabs(Root.real))
        {
            Root.imag=0.0;
        }
        Roots[j-1]=Root;
