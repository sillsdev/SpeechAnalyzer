...
// Validate requested spectrum settings and signal parameters.            
 if (!ppoSpectrum) return(Code(INVALID_PARM_PTR));  //address of pointer to spectrum object 
                                                    //  must not be NULL
 *ppoSpectrum = NULL;                                                    
...
// Allocate memory to contain raw and smoothed power spectra.
 ...
 if (!pfRawSpectrum) return(Code(OUT_OF_MEMORY));
...
{
 if (!stFrame.Start) return(Code(INVALID_PARM_PTR));
 if (!stFrame.Length) return(Code(INVALID_SIG_LEN));        
...
#define pfCepstralCoeff  m_pfFFTBuffer
  ...
  {
    if (pfCepstralCoeff[i] > pfCepstralCoeff[i+1]) nDirection = FALLING_TREND;
    else if (pfCepstralCoeff[i] < pfCepstralCoeff[i+1]) nDirection = RISING_TREND;
    else nDirection = LEVEL_TREND;
    if (nDirection != nTrend[0])
    {
      nTrend[2] = nTrend[1];
      nTrend[1] = nTrend[0];
      nTrend[0] = nDirection;
      if (nTrend[0] == LEVEL_TREND) wCorner = (uint16)(i + 1);
  ...
  else m_pstFormant[0].FrequencyInHertz = UNVOICED;
  ...
  for (i = 1, j = 1; i < m_wFFTLength/2; i++)
  {
    if (j <= m_wNumFormants)
    {
      if (pfSpectCoeff[i].real > pfSpectCoeff[i-1].real) nDirection = RISING_TREND;
      else if (pfSpectCoeff[i].real < pfSpectCoeff[i-1].real) nDirection = FALLING_TREND;
      else nDirection = LEVEL_TREND;
      if (nDirection != nTrend[0])
      {
        nTrend[2] = nTrend[1];
