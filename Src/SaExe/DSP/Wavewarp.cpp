...
// Validate parameters passed.
 if (!ppWaveWarp || !pDoc) return(Code(INVALID_PARM_PTR));   
  *ppWaveWarp = NULL;
 FmtParm *pstSmpFormat = pDoc->GetFmtParm();
 ULONG dwWaveLength = pDoc->GetDataSize() / pstSmpFormat->wBlockAlign;
 if (dwWaveStart >= dwWaveLength) return(Code(INVALID_BLOCK));
 if (!wSpeed) return(Code(INVALID_WARP_SPEED));   
...
// Locate fragment containing starting position for waveform warping.
 ...
 if (!pstFragBfr) return(Code(FRAGMENT_NOT_FOUND));
...
// Find fragment containing the wave sample where warping is to start.
 do{                                              
    ...
    for (dwFragBfrIndex = 0; dwFragBfrIndex < dwFragBlockLength; dwFragBfrIndex++)
         if (dwWaveStart >= pstFragBfr[dwFragBfrIndex].dwOffset && 
             dwWaveStart < pstFragBfr[dwFragBfrIndex].dwOffset + pstFragBfr[dwFragBfrIndex].wLength) break;
 ...
 if (dwFragBlock == dwFragCount) return(Code(FRAGMENT_NOT_FOUND));
 ...
 *ppWaveWarp = new CWaveWarp(pDoc, dwWaveStart, wSpeed, dwFragBlock + dwFragBfrIndex);
 if (!*ppWaveWarp) return(Code(OUT_OF_MEMORY));
...
{ 
 if (!pWaveBfr) return(Code(INVALID_PARM));
...
{ 
 if (!pPlayBfr || !dwPlayBfrLength) return(Code(INVALID_PARM));
...
{ 
 ...
 if (!m_pWaveBfr || !m_pPlayBfr) return(Code(INVALID_PARM));          //!!is this necessary?
 CProcessFragments *pFragments = m_pDoc->GetFragments();
 if (pFragments->GetProcessBufferIndex() != m_dwFragBlock) pFragments->GetFragmentBlock(m_dwFragBlock);  // reload block
 if (dwWaveBlock != m_pstFragBfr[m_dwFragBfrIndex].dwOffset) return(Code(INVALID_BLOCK));
    ...
    do{                                              
       ...
       for ( ; m_dwFragBfrIndex < dwFragBlockLength; m_dwFragBfrIndex++)  // continues from last fragment checked
            if (dwWaveIndex >= m_pstFragBfr[m_dwFragBfrIndex].dwOffset && 
                dwWaveIndex < m_pstFragBfr[m_dwFragBfrIndex].dwOffset + m_pstFragBfr[m_dwFragBfrIndex].wLength) break;
 ...
 /***************************** DEBUG ONLY *************************************/ 
        #ifdef DUMP
           if (m_sbSmpDataFmt == 2)
               for (DWORD i =0; i < m_dwPlayLength; i++) fprintf(m_hPlay, "%d ", ((short *)m_pPlayBfr)[i]);
        #endif
 /******************************************************************************/       
...
{ 
 ...
 if (!m_pWaveBfr || !m_pPlayBfr) return(Code(INVALID_PARM));
 CProcessFragments *pFragments = m_pDoc->GetFragments(); 
 if (pFragments->GetProcessBufferIndex() != m_dwFragBlock) pFragments->GetFragmentBlock(m_dwFragBlock);  // reload block 
 if (dwWaveBlock != m_pstFragBfr[m_dwFragBfrIndex].dwOffset) return(Code(INVALID_BLOCK));
    ...
    // Copy fragment.  
     if (m_sbSmpDataFmt == PCM_UBYTE)
     {
      ...
      for (DWORD i = 0; i < dwFragLength; i++)
      {
       ...
       if (fSample > 127.F) fSample = 127.F;
       else if (fSample < -128.F) fSample = -128.F;
     ...
     {            
      ...
      for (DWORD i = 0; i < dwFragLength; i++)
      {
       ...
       if (fSample > 32767.F) fSample = 32767.F;
       else if (fSample < -32768.F) fSample = -32768.F;
       *pPlay++ = (short)fSample;
      }
     }                
     m_dwPlayLength += dwFragLength;    // update playback length
            ...
            if (lWaveBfrIndex)
               {
                ...
                if (dSmpDiff == 0.) m_dFragStartTime -= 0.5;
                // Otherwise, estimate zero crossing using a linear interpolation method.  
                else m_dFragStartTime -=  (double)(((uint8 *)m_pWaveBfr)[lWaveBfrIndex] - 128) / dSmpDiff;
            ...
            if (lWaveBfrIndex < (long)dwWaveBlockLength && m_dwFragBfrIndex < dwFragCount)
               {
                ...
                dSmpDiff = (double)((uint8 *)m_pWaveBfr)[lWaveBfrIndex] - (double)((uint8 *)m_pWaveBfr)[lWaveBfrIndex - 1];
                // If no difference, assume zero crossing is halfway between samples.                
                if (dSmpDiff == 0.) m_dFragEndTime -= 0.5;                  
                // Otherwise, estimate zero crossing using a linear interpolation method.                  
                else m_dFragEndTime -= (double)(((uint8 *)m_pWaveBfr)[lWaveBfrIndex] - 128) / dSmpDiff;
               }                   
           } 
        else  // PCM_2SSHORT
           {
            ...
            if (lWaveBfrIndex)
               {                     
                ...
                if (dSmpDiff == 0.) m_dFragStartTime -= 0.5;                   
                // Otherwise, estimate zero crossing using a linear interpolation method.  
                else m_dFragStartTime -= (double)((short *)m_pWaveBfr)[lWaveBfrIndex] / dSmpDiff; 
               }                    
            // Initialize to first sample location beyond end of fragment.   
            m_dFragEndTime = (double)(m_pstFragBfr[m_dwFragBfrIndex].dwOffset + m_pstFragBfr[m_dwFragBfrIndex].wLength);
            // Interpolate between last sample of current fragment and first sample of next.
            ...
            if (lWaveBfrIndex < (long)dwWaveBlockLength && m_dwFragBfrIndex < dwFragCount)
               {
                ...
                if (dSmpDiff == 0.) m_dFragEndTime -= 0.5;                  
                // Otherwise, estimate zero crossing using a linear interpolation method.                  
                else m_dFragEndTime -= (double)((short *)m_pWaveBfr)[lWaveBfrIndex] / dSmpDiff;
               }                   
           }   
