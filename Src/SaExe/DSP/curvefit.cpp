...
{
  if (!ppCurveFitter) return(Code(INVALID_PARM_PTR));
  float *pCoeff = (float *)new float[CurveType];
  if (!pCoeff) return(Code(OUT_OF_MEMORY));
...
{
  if (!Data) return(Code(INVALID_PARM_PTR));
  if (!nDataPoints) return(Code(INVALID_DATA_LEN));
  switch(m_nCurveType)
  {
    case PARABOLIC:
      if (nDataPoints < 3) return(Code(INVALID_DATA_LEN));
      if (nDataPoints == 3)
      { // exact fit
        ////////////////////////////////////////////////////////////////////////
...
{
  if (!m_pData) return(Code(INVALID_PARM_PTR));
  switch(m_nCurveType)
  { 
    case PARABOLIC:
...
{
  if (!m_pData) return(Code(INVALID_PARM_PTR));
  if (!x) return(Code(INVALID_PARM_PTR));
  switch(m_nCurveType)
  { 
    case PARABOLIC:
...
{
  if (!m_pData) return(Code(INVALID_PARM_PTR));
  if (!pFirstDerivRoot) return(Code(INVALID_PARM_PTR));
  switch(m_nCurveType)
  { 
    case PARABOLIC:
       if (m_coeff[2] == 0.F) 
