...
{
  if (!ppPeakPicker) return(Code(INVALID_PARM_PTR));
  *ppPeakPicker = NULL;
  if (!dwMaxNumBumps) return(Code(INVALID_NUM_PEAKS));
  BUMP_TABLE_ENTRY *BumpTable = (BUMP_TABLE_ENTRY *)malloc((int32)dwMaxNumBumps * sizeof(*BumpTable));
  if (!BumpTable) return(Code(OUT_OF_MEMORY));
  ...
  *ppPeakPicker = new CPeakPicker(BumpTable, dwMaxNumBumps, pCurveFitter);
  if (!*ppPeakPicker) return(Code(OUT_OF_MEMORY));
  return(DONE);
}
...
CPeakPicker::CPeakPicker(uint32 dwMaxNumBumps)
{
  ...
  if (m_dwMaxNumBumps) m_BumpTable = (BUMP_TABLE_ENTRY *)malloc((int32)dwMaxNumBumps * sizeof(*m_BumpTable));
...
{
  if (!m_pParabola || !m_BumpTable) return(Code(OUT_OF_MEMORY));
  return(DONE);
}
////////////////////////////////////////////////////////////////////////////////////////
...
{
  if (!pPeakCount) return(Code(INVALID_PARM_PTR));
  *pPeakCount = 0;
  if (!pPeakTable) return(Code(INVALID_PARM_PTR));
  if (!pSignal) return(Code(INVALID_SIG_DATA_PTR));
  if (!dwSignalLength) return(Code(INVALID_SIG_LEN));
  ...
  for (float *y = pSignal+1; y < pEnd; y++)
  {
    if (y[0]>y[-1] && y[0]>=y[1])
    {
      ...
      if (!Err)
      {
        ...
        if (++m_dwBumpCount == m_dwMaxNumBumps) break;
      }
    }
  }
...
dspError_t CPeakPicker::GetBumps(BUMP_TABLE_ENTRY **ppBumpTable, uint32 *pBumpCount,
                          float *pSignal, uint32 dwSignalLength)
{
  if (!pBumpCount) return(Code(INVALID_PARM_PTR));
  *pBumpCount = 0;
  if (!ppBumpTable) return(Code(INVALID_PARM_PTR));
  if (!pSignal) return(Code(INVALID_SIG_DATA_PTR));
  if (dwSignalLength < 3) return(Code(INVALID_SIG_LEN));
  ...
  { dspError_t RetValue = FindNextBump(&Bump, &y, pEnd);
    if (RetValue == BUMP_NOT_FOUND) break;
    if (RetValue != DONE) return(RetValue);
...
dspError_t CPeakPicker::FindNextBump(BUMP_TABLE_ENTRY *pBump, const float **ppStart, const float * const pEnd)
{
  ...
  *ppStart = y;
  ...
  if (pBump->Distance == UNDEFINED_DATA) return(BUMP_NOT_FOUND);    //!!Code?
  return(DONE);
}
