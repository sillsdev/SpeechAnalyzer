...
{
  if (!ppLpcObject) return(Code(INVALID_PARM_PTR));
  *ppLpcObject = NULL;
  
  if (!Signal.Start) return(Code(INVALID_SIG_DATA_PTR));
  if (!Signal.Length) return(Code(INVALID_SIG_LEN));
  if (Signal.SmpRate < 1) return(Code(INVALID_SMP_RATE));
  ...
  case LPC_CEPSTRAL:
    ...
    //a matrix element
    if (!LpcParm.ppCovarMatrix) return(Code(OUT_OF_MEMORY));
    ...
    if (!LpcParm.pScratchArray) RetMemErr;
    break;
  default:
  ...
  if (LpcParm.Process.ParmSet.bFormants)
  {
    ...
    if (!LpcParm.Model.pPole) RetMemErr;
    ...
    if (!LpcParm.Process.ParmSet.bPwrSpectrum) LpcParm.Process.ParmSet.bPwrSpectrum;  // need power spectrum
    // for peak picking in
    // case root finding fails
  }
  
  if (LpcParm.Process.ParmSet.bMeanSqPredError) LpcParm.Process.ParmSet.bResidual = TRUE;
  ...
  if (LpcParm.Process.ParmSet.bResidual) 
  {
    ...
    if (!LpcParm.Model.pResidual) RetMemErr;
  }
  if (LpcParm.Process.ParmSet.bPredSignal)
  {
    ...
    if (!LpcParm.Model.pPredValue) RetMemErr;
  }
  if (LpcParm.Process.ParmSet.bPwrSpectrum) 
  {
    ...
    if (!LpcParm.Model.pPwrSpectrum) RetMemErr;
  }  
  
  *ppLpcObject = new CLinPredCoding(LpcParm, Signal, wFFTLength);
  if (!*ppLpcObject) RetMemErr;
  return(DONE);   
}
...
{
  if (LpcParm.ppCovarMatrix) delete [] LpcParm.ppCovarMatrix;                         
  if (LpcParm.pScratchArray) delete [] LpcParm.pScratchArray;                       
  if (LpcParm.Model.pFrameBfr) delete [] LpcParm.Model.pFrameBfr;                       
  if (LpcParm.Model.pWinFrameBfr) delete [] LpcParm.Model.pWinFrameBfr;                       
  if (LpcParm.Model.pReflCoeff) delete [] LpcParm.Model.pReflCoeff;                 
  if (LpcParm.Model.pPredCoeff) delete [] LpcParm.Model.pPredCoeff;                 
  if (LpcParm.Model.pLpcCoeff) delete [] LpcParm.Model.pLpcCoeff;                 
  if (LpcParm.Model.pCorrValue) delete [] LpcParm.Model.pCorrValue;                   
  if (LpcParm.Model.pLogAreaRatio) delete [] LpcParm.Model.pLogAreaRatio;           
  if (LpcParm.Model.pNormCrossSectArea) delete [] LpcParm.Model.pNormCrossSectArea; 
  if (LpcParm.Model.pPole) delete [] LpcParm.Model.pPole; 
  if (LpcParm.Model.pPredValue) delete [] LpcParm.Model.pPredValue;
  if (LpcParm.Model.pResidual) delete [] LpcParm.Model.pResidual;
  if (LpcParm.Model.pPwrSpectrum) delete [] LpcParm.Model.pPwrSpectrum;
}
...
{
  if (!ppLpcModel) return(Code(INVALID_PARM_PTR));
  *ppLpcModel = NULL;
  if (m_Signal.SmpDataFmt != PCM_UBYTE) return(Code(INVALID_SMP_DATA_FMT));
  ...
  if (m_LpcParm.Process.ParmSet.bEnergy) CalcEnergy();    
  if (m_LpcParm.Process.ParmSet.bPredSignal) CalcPredSignal();
  if (m_LpcParm.Process.ParmSet.bResidual) CalcResidual();    
  if (m_LpcParm.Process.ParmSet.bMeanSqPredError) CalcMeanSqError();   
  if (m_LpcParm.Process.ParmSet.bNormCrossSectAreas) CalcCrossSectAreas();      
  if (m_LpcParm.Process.ParmSet.bPwrSpectrum) CalcPowerSpectrum(); 
  if (m_LpcParm.Process.ParmSet.bFormants) CalcFormants();  //must follow power spectrum calculation
...
{
  if (!ppLpcModel) return(Code(INVALID_PARM_PTR));
  if (m_Signal.SmpDataFmt != PCM_2SSHORT) return(Code(INVALID_SMP_DATA_FMT));
...
{
 if (pFrame == (uint8 *)m_Signal.Start) m_LpcParm.Model.pFrameBfr[0] = ((short)(pFrame[0] - 128));
 else m_LpcParm.Model.pFrameBfr[0] = (short)Round((double)((short)(pFrame[0] - 128)) - DECAY*(double)((short)(pFrame[-1]-128)));
...
{                              
  if (pFrame == (short *)m_Signal.Start) m_LpcParm.Model.pFrameBfr[0] = pFrame[0];
  else m_LpcParm.Model.pFrameBfr[0] = (short)Round((double)pFrame[0] - DECAY*(double)pFrame[-1]);
  for (USHORT i = 1; i < m_LpcParm.Model.nFrameLen; i++)
...
void CLinPredCoding::ApplyWindow()
{
  ...
  const double *Window = cWindow.WindowDouble();
	...
	USHORT i = 0;
	for ( ; i < m_LpcParm.Model.nFrameLen; i++) 
  {
...
void CLinPredCoding::RemoveDcBias()
{
 ...
 { 
   ...
   else if (dData < -32768.) dData = -32768.;
   else m_LpcParm.Model.pFrameBfr[i] = (short)dData;
 }     
}
...
void CLinPredCoding::CalcReflCoeff(void)
{
  if(m_LpcParm.Model.nMethod == LPC_COVAR_LATTICE)
  {
    ...
    for (k = 0; k < m_LpcParm.Model.nOrder; k++)
    {
      if (k == 0)
      {
        ...
        if (c != 0.) m_LpcParm.Model.pReflCoeff[k] = -2. * c/fplusb;
  ...
  else
  {
    ...
    m_LpcParm.Model.pPredCoeff[0] = 1.;
		...
		int i=1;
		for(; i<=m_LpcParm.Model.nOrder; i++)
    {
...
extern "C" int cdecl Compare(const void *pPole1, const void *pPole2)
{
 ...
 if (dPhaseDiff > 0.) return(GREATER);
 if (dPhaseDiff < 0.) return(LESS);
 // Then by increasing magnitude.
 double dMagDiff = ((COMPLEX_POLAR_FLOAT*)pPole1)->mag - ((COMPLEX_POLAR_FLOAT*)pPole2)->mag;
 if (dMagDiff > 0.) return(GREATER);
 if (dMagDiff < 0.) return(LESS);
 return(EQUAL);
}
...
void CLinPredCoding::CalcFormants(void)
{
  ...
  { 
    ...
    fclose(BumpsDump);
#endif
    if (pFormantPicker) delete pFormantPicker;
  ...
  // bandwidths as well.
  Err = CalcPoles();
  if (Err) return;  // return with formant estimates from spectrum if pole calculation fails
  ...
  unfilteredFormants.reserve((unsigned int)(m_LpcParm.Model.nPoles)); // reserve space to prevent reallocation
	...
	USHORT i = 0;
	for (; i < m_LpcParm.Model.nPoles; i++)
  {
    FORMANT_VALUES formant;
  ...
  if(unfilteredFormants.size())
  {
    ...
    assessment.assign(unfilteredFormants.size(),defaultAssessment);
		...
		const uint16 maxK = m_wFFTLength/2 -1;
...
{
  if (!m_LpcParm.Model.nPredValues) CalcPredSignal();
...
{
 if (!m_LpcParm.Model.nResiduals) CalcResidual();
...
void CLinPredCoding::CalcPowerSpectrum(void)
{
 ...
 for ( ; i < m_wFFTLength; i++) LpcCoeff[i] = 0.F;
...
{
 // Validate parameters.
 if (!wSpectLength || wSpectLength > m_wFFTLength / 2) return(Code(INVALID_SPECT_LEN)); // limit to half FFT length
 if (nScaleSelect != LINEAR && nScaleSelect != DB) return(Code(INVALID_SCALE));  // allow linear or log scales only
 
 // Check calling sequence.
 if (!m_LpcParm.Process.ParmSet.bPwrSpectrum) return(Code(OUT_OF_SEQUENCE)); 
...
// Average the power for all points in each frequency band.
 ...
 double    dAccumPower = dPower;
	...
	short i = 0;
	short j = 0;
	for (; i < (short)m_LpcParm.Model.nSpectValues; i++)
     {
  ...
  {
    if (MaxSpectValue < m_LpcParm.Model.pPwrSpectrum[i]) maxSpec = m_LpcParm.Model.pPwrSpectrum[i];
