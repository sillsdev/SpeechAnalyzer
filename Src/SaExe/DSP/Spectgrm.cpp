...
dspError_t Spectrogram::ValidateSignalParms(SIG_PARMS Signal)
{
	...
	if (!Signal.Start) return(Code(INVALID_SIG_DATA_PTR)); //signal data starting address
	//  must not be NULL
	if (!Signal.Length) return(Code(INVALID_SIG_LEN));     //number of samples must not be
	//  NULL
...
dspError_t Spectrogram::ValidateSettings(SPGM_SETTINGS Setting)
{
	...
	if (!Setting.FreqCnt) return(Code(INVALID_SPGM_HGT));  //requested number of points in
	...
	if (!Setting.SpectCnt) return(Code(INVALID_NUM_SPECTRA)); //requested number of spectra
	...
	if (!Setting.SigBlkLength) return(Code(INVALID_BLOCK_LEN));  //block size of spectra to
...
Spectrogram::~Spectrogram()
{
	...
	if (m_ScreenData) delete [] m_ScreenData;
	if (m_SpgmData) free(m_SpgmData);
	if (m_FmntData) free(m_FmntData);
	if (m_WinFrame) delete [] m_WinFrame;
#ifdef DEBUG
	fclose(PitchDump);
	fclose(CepstraDump);
#endif
								   ...
								   uint8 *MaxPwr)
{
	...
	if (!FreqScale) return(Code(INVALID_PARM_PTR));
	*FreqScale = m_FreqScale;
	if (!SpectScale) return(Code(INVALID_PARM_PTR));
	*SpectScale = m_SpectScale;
	if (!SpectRes) return(Code(INVALID_PARM_PTR));
	*SpectRes = m_SpectScale;
	if (!SpectIntv) return(Code(INVALID_PARM_PTR));
	*SpectIntv = m_SpectIntv;
	if (!MaxPwr) return(Code(INVALID_PARM_PTR));
	*MaxPwr = m_MaxPwr;
...
{
	if (!Power) return(Code(INVALID_PARM_PTR));
	...
	if (SpgmX >= m_SpectCnt || SpgmY >= m_SpgmHgt) return(Code(INVALID_SPGM_COORD));
	*Power = (short)*(m_SpgmData + (uint32)SpgmX * (uint32)m_SpgmHgt + SpgmY);
	if (*Power == BLANK) *Power = NA;
	return(DONE);
}
...
{
	if (!Freq) return(Code(INVALID_PARM_PTR));
	if (m_Status < FORMANTS_CALC)
	{
		Freq->F[0] = (float)NA;
		return(Code(OUT_OF_SEQUENCE));
	}
	if (SpgmX >= m_SpectCnt) return(Code(INVALID_SPGM_COORD));
...
dspError_t Spectrogram::Generate(void)
{
	...
	if (m_Status == SPGM_NOT_READY) return(Code(NOT_READY));      //if not ready, exit
	...
	else NumSpect = (uint32)m_SpectBatchLength;     //otherwise, set to default value
	...
	*/     m_Status = FORMANTS_CALC;
	...
	// Return.
	if (m_SpectToProc != m_SpectCnt) return(PENDING);  //indicate spectrogram still incomplete
...
{
	for (uint16 i = 0; i < m_SpgmHgt; i++)  Spect[i] = BLANK;
	return;
}
...
dspError_t Spectrogram::CalcSpectra(short *BlockStart, uint32 NumSpect)
{
	...
	if (m_FmntTrackSw == true && m_NBWindow.Length() > m_Window.Length()) WinMargin = m_NBWindow.Length() / 2;
...
dspError_t Spectrogram::PreProc(short *Frame, DspWin &Window)
{
	...
	if (m_PreEmphSw == true)
	{
		...
		else m_WinFrame[0] = WData[0] * ((float)Frame[0] - (float)Frame[-1]);
...
#define SBFrame(i)  (int8)(Frame[i] - 128)  //macro to convert from unsigned to signed byte
	...
	if (m_PreEmphSw == true)
	{
		...
		else m_WinFrame[0] = WData[0] * ((float)SBFrame(0) - (float)SBFrame(-1));
...
{
	// Ensure call is valid.
	if (m_Status == SPGM_NOT_READY) return(Code(OUT_OF_SEQUENCE));     //if not ready, exit
	...
	//  formant sets left
	else NumFmntSets = (uint32)m_SpectBatchLength;  //otherwise, set to default value
	...
	// Return.
	if (m_FmntSetToProc != m_SpectCnt) return(PENDING);  //indicate formant tracking still incomplete
...
{
	for (uint16 i = 0; i <= MAX_NUM_FORMANTS; i++)  Fmnt->F[i] = (float)NA;
	return;
}
