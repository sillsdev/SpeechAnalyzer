...
public:
  ...
  // getters   
  BOOL IsGraphSelected(int nGraph) { return m_bIsGraphSelected[nGraph]; };
  BOOL IsRecGraphSelected()        { return m_bIsRecGraphSelected; };
  int NumGraphsSelected(void)  { return m_NumGraphsSelected; };
  int NumRows(void) { return m_NumRows; };
  int NumCols(void) { return m_NumCols; };
  int LayoutType(void) { return m_LayoutType; };
  BOOL bUseSmallFonts(void) { return m_bUseSmallFonts; };
  BOOL bFixedAspectR(void) { return m_bFixedAspectR; };
  BOOL bIsLandscape(void) { return (m_IsLandscape != 0); };  
  BOOL bIsHiRes(void)   { return (m_IsHiRes != 0); };
  BOOL UserClickedPrint(void) { return m_bUserClickedPrint; }; 
  BOOL UserClickedPreview(void) { return m_bUserClickedPreview; };
  static const int LAYOUT_AS_SCREEN()     { return 0; };
  static const int LAYOUT_1GRAPH_PER_PAGE()   { return 1; };
  static const int LAYOUT_CUSTOM()      { return 2; };
...
protected:
  ...
  afx_msg void OnMore(); 
  afx_msg void OnUseScreenLayout()  { EnableCustomLayout(FALSE); };
  afx_msg void OnPrintOnePerPage()  { EnableCustomLayout(FALSE); }
  afx_msg void OnPrintManyPerPage() { EnableCustomLayout(TRUE); }
