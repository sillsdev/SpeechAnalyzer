...
void CDlgCharChart::SetInitialState(CFont* pFont, int nCharTable, CString* pszText)
{
  ...
  if (pszText) m_szString = *pszText;
  // prepare the font for the characters
  LOGFONT logFont;
  if (!m_pFont)
  {
    CFont* pFont = CFont::FromHandle((HFONT)::GetStockObject(SYSTEM_FONT));
    if (pFont) pFont->GetObject(sizeof(LOGFONT), (void*)&logFont); // fill up logFont
  }
  else m_pFont->GetObject(sizeof(LOGFONT), (void*)&logFont); // fill up logFont
...
void CDlgCharChart::vSetupControls()
{
  ...
  // move checkboxes over buttons
	CRect rCntrl, rDlg;
	int nTextWidth = 65; // Approximate size of segment checkbox in pixels
  ...
  {
    LPCTSTR szText[kPlayButtons] = { _T("Sound"), _T("Word"), _T("Segment") };
  ...
  wndEdit.SetWindowText(m_szString);
  if (m_pFont) wndEdit.SetFont(m_pFont);
