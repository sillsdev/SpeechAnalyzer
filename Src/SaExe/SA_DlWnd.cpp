...
void CLEDDisplay::SetTime(int nMin, int nSec)
{
  ...
  if (chOldMinMSD != m_chMinMSD) InvalidateRect(rWnd, FALSE); // value changed
  else
  {
    rWnd.left += LED_DIGIT_WIDTH + LED_DIGIT_SPACE;
    if (chOldMinLSD != m_chMinLSD) InvalidateRect(rWnd, FALSE); // value changed
    else
    {
      rWnd.left += LED_DIGIT_WIDTH + LED_POINT_WIDTH + 2 * LED_DIGIT_SPACE;
      if (chOldSecMSD != m_chSecMSD) InvalidateRect(rWnd, FALSE); // value changed
      else
      {
        rWnd.left += LED_DIGIT_WIDTH + LED_DIGIT_SPACE;
        if (chOldSecLSD != m_chSecLSD) InvalidateRect(rWnd, FALSE); // value changed
        else
        {
          rWnd.left += LED_DIGIT_WIDTH + LED_POINT_WIDTH + 2 * LED_DIGIT_SPACE;
          if (chOldSecDec != m_chSecDec) InvalidateRect(rWnd, FALSE); // value changed
          else return;
        }
      }
    }
...
void CSliderVertical::SetPosition(int nVal)
{
  ...
  if (m_chPos > 100) m_chPos = 100;
  {
    ...
    m_rPos.SetRect((rWnd.right - m_rPos.Width()) / 2, nPos,
      ...
      nPos + m_rPos.Height());
    if (rOld.left == 0) InvalidateRect(NULL, TRUE); // first paint, paint all new
    else
    {
...
int CSliderVertical::GetPosFromMouse(CPoint point)
{
  ...
  if (rWnd.top > 100) rWnd.top = 100;
  if (rWnd.top < 0) rWnd.top = 0;
  return rWnd.top;
}
...
int CSliderHorizontal::GetPosFromMouse(CPoint point)
{
  ...
  if (rWnd.left > 100) rWnd.left = 100;
  if (rWnd.left < 0) rWnd.left = 0;
  return rWnd.left;
}
...
void CVUBar::SetVU(int nLVal, int nRVal)
{
  ...
  if (nLVal > 100) nLVal = 100; // limit new left value
  // change new left value to number of displayed divisions
  m_chLVU = char((nLVal + (50 / VU_BAR_DIVISIONS)) / (100 / VU_BAR_DIVISIONS));
  if (nRVal == -1) m_chRVU = m_chLVU; // get new right value from left for mono
  else
  {
    if (nRVal > 100) nRVal = 100;
  ...
  if (rWnd.right < rWnd.bottom) // vertical bar
  {
    ...
    if ((m_chLOldVU == m_chLVU) && (m_chROldVU == m_chRVU)) return; // no change
...
void CVUBar::OnPaint()
{
  ...
  if (rWnd.right < rWnd.bottom)
  {
    ...
    if (nLoop >= VU_BAR_GREEN) dc.SelectObject(&brushGreen); // select green brush	
    if (nLoop >= VU_BAR_YELLOW) dc.SelectObject(&brushYellow); // select yellow brush
    if (nLoop >= VU_BAR_RED) dc.SelectObject(&brushRed); // select red brush
    int nLimit = m_chLVU;
    if (nLimit < m_chRVU) nLimit = m_chRVU;
    for (nLoop; nLoop < nLimit; nLoop++)
    {
      ...
      if (nLoop == VU_BAR_YELLOW) dc.SelectObject(&brushYellow); // select yellow brush
      if (nLoop == VU_BAR_RED) dc.SelectObject(&brushRed); // select red brush
      if (nLoop < m_chLVU) dc.Rectangle(rDraw); // draw one of the left divisions
      rDraw.OffsetRect(VU_CHANNEL_DISTANCE + VU_BAR_WIDTH, 0); // move draw rectangle to right channel
      if (nLoop < m_chRVU) dc.Rectangle(rDraw); // draw one of the right divisions
    ...
    if (m_bPeak)
    {
      if (m_chLPeak > (m_chLVU + 1))
      {
		...
		if (m_chLPeak >= VU_BAR_GREEN) dc.SelectObject(&brushGreen); // select green brush	
        if (m_chLPeak > VU_BAR_YELLOW) dc.SelectObject(&brushYellow); // select yellow brush
        if (m_chLPeak > VU_BAR_RED) dc.SelectObject(&brushRed); // select red brush
        dc.Rectangle(rDraw);
      }
      if (m_chRPeak > (m_chRVU + 1))
      {
        ...
        rDraw.bottom = rDraw.top + 3;
		if (m_chRPeak > VU_BAR_GREEN) dc.SelectObject(&brushGreen); // select green brush	
        if (m_chRPeak > VU_BAR_YELLOW) dc.SelectObject(&brushYellow); // select yellow brush
        if (m_chRPeak > VU_BAR_RED) dc.SelectObject(&brushRed); // select red brush
        dc.Rectangle(rDraw);
      }
    }
  }
...
void CProgressBar::SetProgress(int nVal)
{
  ...
  if (nVal > 100) nVal = 100; // limit new value
  ...
  if (chOldProgress == m_chProgress) return; // no change
  CRect rWnd, rUpdate;
  GetClientRect(rWnd);
...
void CSpinControl::DrawArrowUp(CDC* pDC, CRect rWnd)
{
  ...
  if (!IsWindowEnabled()) pDC->SelectObject(pOldPen);
}
...
void CSpinControl::OnPaint()
{
  ...
  if (m_bUpButtonDown) DrawButtonDown(&dc, rButton);
  else DrawButtonUp(&dc, rButton);
  DrawArrowUp(&dc, rButton);
  // paint lower button up
  rButton.SetRect(rWnd.left, rWnd.bottom / 2, rWnd.right, rWnd.bottom);
  if (m_bLowButtonDown) DrawButtonDown(&dc, rButton);
  else DrawButtonUp(&dc, rButton);
...
void CToggleButton::DrawText(CDC* pDC, CRect rWnd)
{
  ...
  // set font color
  if (IsWindowEnabled()) pDC->SetTextColor(GetSysColor(COLOR_BTNTEXT));
...
void CToggleButton::OnPaint()
{
  ...
  GetClientRect(rWnd);
  // paint button
  if (m_bButtonDown) DrawButtonDown(&dc, rWnd);
  else DrawButtonUp(&dc, rWnd);
}
