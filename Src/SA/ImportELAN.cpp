...
CImportELAN::CImportELAN( const CSaString & szFileName, BOOL batch)
{
    m_szPath = szFileName;
    m_bBatch = batch;
}
...
/***************************************************************************/
void CImportELAN::AutoAlign(CSaDoc * pSaDoc, LPCTSTR pReference, LPCTSTR pPhonetic, LPCTSTR pPhonemic, LPCTSTR pOrtho, LPCTSTR pGloss)
{
...
*/
BOOL CImportELAN::ProcessColumnar( wistringstream & stream, wstring & result)
{
	// rewind the stream
	stream.clear();
	stream.seekg(0);
...
bool CImportELAN::ProcessNormal( wistringstream & data, EImportMode nMode, wstring & result)
{
    CSaString text;
	CSaString ref;
    CSaString phonetic;
