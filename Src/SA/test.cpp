/////////////////////////////////////////////////////////////////////////////
// sa_edit.cpp:
// Implementation of the CDlgEditor2 (dialog)
//                       CDlgAnnotationEdit2 (dialog)           classes.
//
...
//      SDM Original
// 1.06.6
//      SDM Added CDlgAnnotationEdit2 Class
// 1.06.6U2
//      SDM Changed CDlgEditor2 to auto hide on Empty documents
// 1.06.6U4
...
// 1.5Test8.5
//      SDM CDlgEditor2 removed autohide restore
//      SDM changed CDlgEditor2::PreTranslateMsg to use document accelerator table
// 1.5Test10.0
...
/////////////////////////////////////////////////////////////////////////////
// CDlgEditor2 message map

BEGIN_MESSAGE_MAP(CDlgEditor2, CDialog)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgEditor2 construction/destruction/creation

/***************************************************************************/
// CDlgEditor2::CDlgEditor2 Constructor
/***************************************************************************/
CDlgEditor2::CDlgEditor2(CWnd * pParent) : CDialog(CDlgEditor2::IDD, pParent) {
    bEditor = FALSE;
    m_pPreviousSaView = NULL;
    m_bActivated = FALSE;
}

BOOL CDlgEditor2::OnCmdMsg(UINT nID, int nCode, void * pExtra, AFX_CMDHANDLERINFO * pHandlerInfo) {
	TRACE("id=%d code=%d\n",nID,nCode);
...
/////////////////////////////////////////////////////////////////////////////
// CDlgEditor2 helper functions

// SDM 1.5Test8.2
/***************************************************************************/
// CDlgEditor2::CreateSafe Create dialog load save position
/***************************************************************************/
BOOL CDlgEditor2::CreateSafe(UINT nIDTemplate, CWnd * pParentWnd, WINDOWPLACEMENT * pWPL) {
    if (!bEditor) {
...
/***************************************************************************/
// CDlgEditor2::OnInitDialog Dialog initialisation
// The dialog is centered over the main frame window.
/***************************************************************************/
BOOL CDlgEditor2::OnInitDialog() {
...
/***************************************************************************/
// CDlgEditor2::DoDataExchange Data exchange
/***************************************************************************/
void CDlgEditor2::DoDataExchange(CDataExchange * pDX) {
...
/***************************************************************************/
// CDlgEditor2::SaView Get a pointer to the active CSaView if any
/***************************************************************************/
CSaView * CDlgEditor2::GetView() const {
...
CSaDoc * CDlgEditor2::GetDoc() const {
    return (GetView()!=NULL) ? GetView()->GetDocument() : NULL;
};

CSegment * CDlgEditor2::GetSelectedSegment() const {
    return (GetView()!=NULL) ? GetView()->FindSelectedAnnotation() : NULL;
};

/***************************************************************************/
// CDlgEditor2::OnCharacterChart Input from character chart wanted
/***************************************************************************/
void CDlgEditor2::OnCharacterChart() {
...
/***************************************************************************/
// CDlgEditor2::OnPlaybackSegment Button playback segment hit
// Sends a message to the player to playback the current segment selection.
/***************************************************************************/
void CDlgEditor2::OnPlaybackSegment() {
    if (!GetView()) {
...
/***************************************************************************/
// CDlgEditor2::OnPlaybackWord Button playback word hit
// Sends a message to the player to playback the original gloss selection.
/***************************************************************************/
void CDlgEditor2::OnPlaybackWord() {
    if (!GetView()) {
        return;    // require doc and view
    }
...
/***************************************************************************/
// CDlgEditor2::OnPlaybackPhraseL1 Button playback Phrase L2 hit
// Sends a message to the player to playback the original phrase selection.
/***************************************************************************/
void CDlgEditor2::OnPlaybackPhraseL1() {
    if (!GetView()) {
        return;    // require doc and view
    }
...
// Sends a message to the player to playback the original phrase selection.
/***************************************************************************/
void CDlgEditor2::OnStopPhraseL1() {
...
/***************************************************************************/
// CDlgEditor2::OnUpdateInputstring Inputstring is changed about to update
// Filter input stream dialog input
/***************************************************************************/
void CDlgEditor2::OnUpdateInputstring() {
...
/***************************************************************************/
// CDlgEditor2::OnUpdateDialog Update dialog to current state
/***************************************************************************/
void CDlgEditor2::UpdateDialog() {
...
/***************************************************************************/
// CDlgEditor2::IsDifferent Checks if selection has changed since last update
/***************************************************************************/
BOOL CDlgEditor2::IsDifferent(BOOL bUpdate) {
    BOOL ret = FALSE;
...
BOOL CDlgEditor2::PreTranslateMessage(MSG * pMsg) {
    ...
    if (pMain && (pMain->GetCurrSaView()) && (pMsg->message >= WM_KEYFIRST && pMsg->message <= WM_KEYLAST)) { 
        ...
        default:
			...
			// 1.5Test8.5
            if (pMain->GetCurrSaView()->PreTranslateMessage(pMsg)) { 
                return TRUE;
            }
			break;
        }
...
/***************************************************************************/
// CDlgEditor2::OnActivate
// Set Focus to edit control, or if disabled to next button
/***************************************************************************/
void CDlgEditor2::OnActivate(UINT nState, CWnd * pWndOther, BOOL bMinimized) {
...
/***************************************************************************/
// CDlgEditor2::OnCancel undo current change and close
/***************************************************************************/
void CDlgEditor2::OnCancel() {
...
// SDM 1.5Test8.2
/***************************************************************************/
// CDlgEditor2::OnClose close TE don't undo current change
/***************************************************************************/
void CDlgEditor2::OnClose() {
    SendMessage(WM_COMMAND, IDOK, 0);
}

/***************************************************************************/
// CDlgEditor2::OnHelpEdit Call Information help
/***************************************************************************/
void CDlgEditor2::OnHelpEdit() {
    // create the pathname
...
CMainFrame * CDlgEditor2::GetMainFrame() const {
    return (CMainFrame *)AfxGetMainWnd();
}

void CDlgEditor2::UpdatePlayer() {
    CMainFrame * pMainWnd = GetMainFrame();
    if (pMainWnd==NULL) {
        return;
    }
...
void CDlgEditor2::SetButtonState(int playButton1, int stopButton, bool play) {
